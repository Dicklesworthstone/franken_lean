...
7. **The recurring defect: evidence must be produced where the claim is made.** Stated once, generally, because it has now been found nine times and every single time by somebody *reading carefully* rather than by a check:
   ...
   | `fln-mandated-mutant-join-unwatched-uagk` | a mandated mutant and the test that kills it | **partly** — `mandated_mutants.rs` derives §18's five names from AGENTS.md at test time rather than transcribing them, and fails four ways: a name neither marked nor declared not-yet-seeded; a not-yet-seeded declaration whose crate has stopped being a stub, which is the reminder firing at the moment the subsystem lands; a declaration that outlived its own repair, so the remainder shrinks; and a scan that returns empty, which is a broken scan rather than a clean tree. It checks **markers, not kills**: a test gutted behind its marker stays green, and that three of three applicable mutants actually die was established once by planting them, not per commit |
   ...
   **Where the luck is still load-bearing.** Six of the nine now fail on recurrence; two are half-open, in different halves; and one has nothing watching it at all. `kl4h`'s movement is mechanised but the contradiction's `Acknowledged` status is not. `uagk` mechanises the join from §18's list to a *marker* and leaves the join from a marker to an actual *kill* unwatched, so a test gutted behind its marker stays green — closing that means running the mutants in CI, which is a mutation-campaign lane rather than a guard. `vdi4` is fully open: 166 anchors are unverifiable from `main` today and nothing would notice a hundred more. Everywhere else the mechanism, not the reviewer, is doing the work. Nine instances found by attentive reading is not a mechanism; it is luck with good people, and luck does not survive a context restart.

   **The two newest rows differ in a way worth keeping.** `bkw6` is a claim whose referent **never existed**; `vdi4` is a claim whose referent **existed and was destroyed wholesale by a tool**. Both defeat the same technique for the same reason — there is nothing addressable to compare a claim against — and neither is reachable by reading one artifact more carefully. The generalisable move is the one `bkw6` used: when the far end is empty or gone, bind the claim to the **cardinality** of what it asserts, and let the number fail in both directions.
