...
> **Put the gate in its own file and invoke it — `python3 -I -S <check-script> && git commit -o …`. Never inline a heredoc into an `&&` chain. Then audit what landed against its parent, because that audit is what separates a disclosed near miss from an undetected one.**
