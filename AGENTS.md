...
**CI walks the first mechanically; the second is *part walked, part declared-and-reviewed*, and this sentence has now been wrong in both directions** (bead `fln-boundary-api-no-admission-argument-discarded-ez07`, audited at `c7a23f02`, repaired in the same commit as this sentence). It first claimed CI walked both, which was false; the correction said the export law was walked *nowhere*, which is no longer true. The **dependency** law is real and *derived*: the edge set comes from the actual `Cargo.toml` files (`checks.rs:1918`), `FLN-STRUCT-008` (`checks.rs:2075`) walks it transitively for every crate matching the `fln-unsafe-*` **pattern** — so a new boundary crate is covered without editing the rule, and a laundering path whose every hop is rank-legal still fails (`seeded.rs:253` plants exactly that, since `fln-unsafe-jit` is rank 12 and `fln-kernel` rank 6, so layering alone would permit it) — and `FLN-STRUCT-024` (`checks.rs:1667`) pins the prohibition itself, so deleting the line fails too. It runs under plain `cargo test` (`real_workspace.rs:50`, not `#[ignore]`d), not only in a lane.
...
5. **The kernel answers to no one (D6, FL-INV-02).** `fln-kernel` is ≤ 12 KLOC, dependency-closure-on-one-page, exporting exactly one authority: `check : Environment × Declaration → Verdict`. Nothing else can admit a constant. Kernel disagreement with the Reference at the pin is release-blocking, with one carve-out: soundness beats bug-parity (D23). CI counts release-compiled authority and walks the graph; an exact top-level `#[cfg(test)] mod tests { ... }` lexical tail is excluded because it is absent from the release TCB, while a compound/nested/non-tail shape fails closed into the count. Growth of release authority requires amending the plan first.
...
**The property underneath this whole section, stated once, because it has two instances and the table below only answers for one.** *A suite that shells out to git needs a real `.git` **directory**.* Two environments a pane is actively encouraged to use do not have one, each for the very reason it is recommended:

| environment | what `.git` is there | why you were sent there |
|---|---|---|
| a linked git **worktree** | a **file** holding a `gitdir:` pointer | to dodge another pane's dirty shared tree |
| an rch **clean-overlay worker** | **absent entirely** | to dodge local build load |

**Measured, by execution rather than by reading the code**: 20 tests fail without a `.git` directory and pass with one — bead `franken_lean-rch-clean-overlay-has-no-git-dir-46pw`, whose comment carries the enumeration. The method is the part to reuse: two trees extracted from **one** `git archive` tar so their contents are identical by construction, differing only in the presence of `.git`, then `cargo test --workspace --no-fail-fast` in each. A test failing in the first and passing in the second failed *because* there was no repository.

**Do not classify these by message.** That was tried first and under-counted by 2.5× — 8 against 20 — because several suites wrap the git failure in their own typed refusal (`GitRefusal { … }`, `rev-list … returned 0 commits`, `the contract has an introduction commit`) that no single needle finds, and a longer needle list is a longer guess. This section already records that the *exit code* discriminates nothing; the sharper statement is that **the message does not either**, and only a positive control — the same tree with a `.git` — separates the two. The 20 is a **floor**: those probes carried no `vendor/` and no untracked census, so a suite dying on an absent input before it reaches its git call is invisible to the method.

**The bitter half:** the better a guard follows this repository's own rule that scope must be *derived* from the repository rather than hand-listed, the less able it is to run in either environment. Three of the 20 were written this week to do exactly that.
...
| **any** `cargo test` invocation **without** `--no-fail-fast` — `--workspace`, `-p A -p B`, or a single `-p` | **do not report a tally from it** — cargo stops at the first failing *target*. **This row said `--workspace` for weeks and the scope was too narrow.** **And the reason `46pw` gives for caring is wrong in one direction, measured at `2e02eb4d`:** the bead says a pane may cite *"a green from a run in which the assertions never executed"*, but fail-fast stops a run only when something **fails** — a green run executed every target either way. Verified on a passing package: bare and `--no-fail-fast` both report the same **14** targets with identical counts and exit 0, differing only in elapsed time. So this flag **cannot manufacture a false green**; the hollow-green risk the bead reaches for is a different mechanism, `fln-rgha`'s pin-absent `typed_skip`, and `ci_execution_join` already guards it. What this row protects is the **red** direction, which is the one that has actually cost time here twice in a day: a failing run's tally read as the whole story. Re-measured at `2e02eb4d` as a one-variable pair — same tree, same commit, same package, only the flag moving — on a **single** `-p`, the form everyone uses and the form `rch` recommends: `--no-fail-fast` reports **678 passed, 18 failed across 61 targets**; the bare form reports **173 passed, 1 failed across 3**. It hides **17 of the 18 failures and 58 of the 61 targets**. The same tree, same commit, reports "101 ok, 1 red": it hides 37 suites and the second failing suite entirely. A test name absent from that output means *did not run*, never *passed*. I published the 101/1 figure before re-running with the flag; this row is that correction |
| `ubs <paths>` — the fourth mandated check | **yes — `cd` into the tree first and pass RELATIVE paths.** `ubs` stages a shadow workspace rooted at **cwd** and cannot stage a path resolving outside it; the trigger is the argument, not the checkout. Measured as a 2×2 at `14638e4c`, same file, worktree `.git` a 59-byte pointer file: cwd=main+relative **0**, cwd=worktree+relative **0**, cwd=main+absolute-into-worktree **1**, cwd=worktree+absolute-into-main **1** (cc_3, reproduced by cc_2). A worktree `ubs` delta at a pinned commit is therefore still available, and is the cleanest attributable baseline while the shared tree is dirty. **But never compare counts across two trees**: a worktree at a clean commit and a main tree carrying WIP hold different bytes, so the gap reads as a tool inconsistency when it is a content difference (56 vs 34 on one file, measured 2026-07-26). Hold the path and the cwd fixed and vary **only** the content — `git show "${sha}:<path>" > <tmp> && mv <tmp> <path>`, scan, restore — which is the same one-variable rule that this row's own first version broke. Two things about that command are load-bearing and both were learned the expensive way. **Never redirect over the file you are protecting**: `>` truncates *before* the producer runs, so any failure — a bad sha, a wrong path, a full disk — leaves you holding 0 bytes, and the earlier version of this row prescribed exactly that. Write to a temp path and move it into place, so a failed producer cannot destroy the original. **And brace and quote the rev exactly as written; do not "simplify" it back**: the tool shell is zsh, where `"$sha:<literal>"` is silently rewritten by modifier expansion whenever the literal's first character is a modifier letter. Derived over the whole ASCII alphabet on 2026-07-28 at zsh 5.9, with bash 5.2 as a control triggering **0** — the trigger set is the **17** letters `a c e f g h l q r s t u w A F P Q`, which in this tree means `ci/`, `contracts/`, `crates/`, `scripts/`, `tools/`, `tribunal/`, `rust-toolchain.toml`, `rustfmt.toml`, `AGENTS.md` and `ABI_CONTRACT.md` — **10 of the 23** tracked top-level entries — while `Cargo.*`, `README.md`, `SUITE.lock`, `LICENSE`, `vendor/` and `.beads/` are untouched. The 43 printable **non-letter** first characters were swept too, each against a braced control in the same harness so an ordinary expansion cannot be miscounted as a rewrite: only `&` mangles, `$` moves in *both* forms and is therefore not attributable to this mechanism, and the other 41 — digits and `.` `_` `-` `~` `@` `+` `#` `%` `,` `=` `^` `!` and the brackets — are **measured safe** rather than merely untested, which is why `.beads/` and `.github/` are genuinely fine. `crates/…` becomes `/usr/bin/HEADrates/…` and `scripts` becomes the **empty string**. `git` then rejects the mangled rev-spec, so the failure is loud unless you have added `2>/dev/null` — which a provenance loop always has. `git show "$sha:$path"`, with the path in a **variable**, is safe, because a `$` is not a modifier letter — which is why a probe that loops the path through a variable reports 12/12 clean and is structurally incapable of reproducing this |
...
| any `fln.e2e/2` lane — **22 declared fln.e2e/2 lanes**, of which **22 refuse on a measured invocation shape** and **0 whose verdict is unmeasured** | **all twenty-two no.** This row's scope and verdict classes are derived per commit by `the_worktree_refusal_scope_is_derived_from_the_lane_population`, and **the derivation is what moved these numbers, not a reading**: the row said nineteen-and-one for two days after `unsafe_note_clippy.sh` stopped being the exception. It was the sole indeterminate lane because it carried no `--vendor-path`; at `017ef65e` and `4f88ab02` it gained both `vendor-binding` and `hash-tree --vendor-path`, so its verdict is now measured like the rest and the six subcommand names that made it unresolvable (`adopt-bundle`, `complete-bundle`, `emit`, `manifest`, `validate-bundle`, `validate-run`) no longer decide anything. All twenty-two now carry `vendor-binding`, whose linked-worktree refusal is measured. **A lane gaining a governed call is the ordinary case and it moves this row every time** — that is the whole point of deriving it. The **35 scripts** in `scripts/e2e/` and the **22 declared lanes** remain different sets. Static reachability currently finds **40 of 50 subcommands** able to reach `run_git`; that fact alone does not prove refusal because `hash-tree` is a measured counterexample that succeeds without `--vendor-path` |
...
**What none of this earns.** RCH lives outside this repository, so no test here can hold its classifier, its sync behaviour or its version — those figures are `bounded_model`, measured at one host at one instant, and the `--clean-overlay` result is one pair of runs on one worker. The original symptom this bead was filed for — the remote holding content *older* than the caller's tree both before and after dispatch — is **not** explained by non-atomic sync, which produces mixtures rather than staleness. **The candidate that was named here as untested is now TESTED, and it reproduces.** Measured on 2026-07-28 against hz2 at rch 1.0.52, in a throwaway worktree under `/data/projects` so no pane could perturb a cell, with `compile_error!` as the observable because rustc quotes the literal and so names exactly which bytes the worker compiled:

| cell | the edit | the worker compiled |
|---|---|---|
| 1 | sentinel `S1` planted, first dispatch | `S1` |
| 2 | changed to `S2`, **size AND mtime both preserved** | **`S1` — the edit never reached the worker** |
| 3 | changed to `S3`, mtime bumped — the control | `S3` |
| 4 | cell 2's condition exactly, but `--base <sha> --clean-overlay --overlay-path` | **`S2` — correct** |

So a content change that leaves size and mtime untouched is **silently not transferred**, and the worker then compiles stale content indefinitely — content older than the caller's tree both before and after dispatch, which is the original symptom's shape and is precisely what non-atomic sync cannot produce. Cell 3 is what makes cell 2 attributable: the same edit with mtime bumped arrives. **State the predicate, not the implementation** — what is measured is *size and mtime unchanged ⇒ not transferred*; that this is rsync's quick check is an inference from the signature, and no cell inspects the transfer.

**Cell 4 is the operational payoff and it strengthens the rule above rather than adding a second one.** `--clean-overlay` is not merely *attributable under concurrent edits*: it is the only measured form that resists content-blind staleness, because it digests the overlay at admission instead of trusting a stat. The default mode's green is therefore weaker than "about the worker's tree" — it can be about a tree state **no host held recently**.

**What this does not earn.** One host, one worker, one rch version, one instant, class `bounded_model`. It establishes the mechanism exists and reproduces on demand; it does **not** establish that the 2026-07-25 instance was caused by it, and that is now unrecoverable. The trigger needs an mtime-preserving write — ordinary editing bumps mtime, and the cells used `touch -r` deliberately — so the real-world frequency is **unmeasured**, and nothing here says which everyday operation supplies it. One further constraint fell out and is worth knowing before anyone designs a probe: rch refuses any project path outside `/data/projects` (`input resolves outside canonical root`) and falls back to local, so an rch experiment **cannot** be isolated under `/data/tmp`.
...
**A THIRD cause for the same two names, measured 2026-08-05, and this one is the gate working rather than anything being wrong.** A full `cargo test --workspace --no-fail-fast` at `82ac0234` reported 227 results ok and exactly one failing target — `structure-guard --test real_workspace`, with *both* `real_workspace_is_structurally_clean` and `robot_real_workspace_binds_complete_authority_evidence` failing, which is precisely the pair the green-bar row above pre-declares red for the untracked census shards. The finding code says otherwise: **`FLN-STRUCT-028` — "governed source root changed during the scan (`fnv1a64:7895bfc3…` -> `fnv1a64:8cfb32a1…`); the run is bound to no single input closure"**, and the guard's own verdict line reads `INCONCLUSIVE — 1 finding(s)`, not a structural finding at all. **Three commits landed during that ~10-minute run**, and the same target scanned alone minutes later was green, 68 ok across both governance packages.

So the cause is neither the census shards nor a defect: it is FL-INV-07's inconclusive family, the guard refusing to bind a verdict to a tree that moved underneath it, which is exactly what it should do. The operational consequence is the part worth carrying — **in this shared checkout a workspace-wide run cannot reliably bind a single input closure, because the swarm commits faster than the run completes.** Re-run the failing target alone before attributing anything; a `structure-guard` red that is `INCONCLUSIVE` with `FLN-STRUCT-028` is a statement about your *run*, not about the repository. Note also what makes this the sharpest instance yet of the rule above: the two test names match the documented cause, the documented cause is real and does still occur, and the reader is therefore confirmed twice over in a diagnosis the finding code refutes in one line.
...
**The residual was measured in this tree and survives the incident that exposed it.** `-o` bounds a commit to the paths you **name**; it does not establish that you **authored** them. The fifth cell: `-o` on a path a peer had edited and I had not commits *their* uncommitted work under my name. On 2026-07-27 two orphaned working-tree files made this concrete, and the `9teu` patch was correctly ruled accepted-on-the-merits but **not landable**, since landing it meant committing 543 uncommitted lines of an absent pane's under the committer's own authorship. `franken_lean-h4o1` later resolved that condition through explicit adoption, a full read, and a new committer taking responsibility for the adopted bytes; absence alone never granted that authority. Derive your path list from what you changed (`git status --porcelain`), and name nothing you did not write.
...
**The fifth residual is not a write at all — it is a READING of the index, and the alarming reading is the wrong one.** Every residual above concerns bytes moving. This one concerns a diff you are about to act on. `git diff --cached HEAD -- <path>` on a path *you* did not stage looks exactly like a peer mid-assembly on your file, and this section has just spent four residuals teaching you to treat that as dangerous — so the reader arrives already holding the frightening interpretation, which is [the wrong-cause shape](#a-block-is-a-claim-and-it-expires--re-test-it-before-you-wait-on-it-and-before-you-act-on-it) supplied by this document rather than by a tool. Measured on 2026-08-05 while landing an unrelated commit: `crates/fln-conformance/tests/evidence_finalization.rs` showed `0/94`, one hunk, reading as a peer staging a **deletion** of a module another pane had added. It was nothing of the kind.

> **Before treating a `--cached` delta on a path you did not stage as a peer's in-flight work, ask whether the staged blob is REACHABLE.** `git fsck --dangling` listing it means somebody staged novel bytes and you are about to discard them. Not listing it means the blob belongs to some commit — a **stale index entry** naming an ancestor's version — and your `git commit -o` refreshing it is a repair, not a casualty.

The cell that settles it costs one command: `git ls-files -s -- <path>` for the staged blob, then look for it in `git fsck --dangling`. In the instance above it was absent from the dangling set and resolved to `810df01e`'s copy of that file — 5373 lines against HEAD's 5467, the 94 being an *earlier* commit's additions rather than anyone's deletion. Five paths were tested this way across one session, including `.beads/issues.jsonl` and `ci/VERIFICATION_MANIFEST.jsonl`; **all five were stale entries and none was a real collision.** The shared index accumulates them because `git commit -o` refreshes only the paths it names, so every path a pane *doesn't* name keeps whatever entry it last had, across dozens of intervening commits by everybody else.

**What this does not earn, and the direction it must not be read in.** It is a *practice*, and nothing runs it for you. It does **not** retire the first residual: a peer staging genuinely novel bytes on a path you also name still loses them to your `-o`, because cell four above measures that the index is updated to match your worktree. This says only how to tell the two apart, and it is one session on one host, class `bounded_model`. A blob can also be reachable *and* freshly staged — someone staging a file back to an older commit's content — so reachable means "no novel bytes are at risk", never "nobody touched it".
...
**This does not soften "never the index", and the distinction is the reason it is safe.** That rule prohibits *assembling a commit* through shared staging — a read-modify-write race whose verification cannot survive the gap between `git add` and `git commit`, measured at the top of this section. `update-index --add` on a path already identical to `HEAD` **assembles nothing**: it stages no content that is not already committed, so there is no window in which a peer's write can change what your commit contains, because you are not making a commit. Note also that the prescription it replaces — `git read-tree HEAD` — writes the shared index too, and over **every** path rather than yours; the recipe has always ended in an index write, and this is the narrow form of one.
...
**It refuses rather than overwrites, and a refusal is usually the CORRECT answer here.** A superseded draft and a peer's in-flight line are indistinguishable by provenance, so it restores only content already present in the pinned `HEAD` blob and refuses outright when the worktree copy carries a line `HEAD` has never seen. In a live swarm that is the normal outcome for `.beads/issues.jsonl` — measured at 21 foreign lines on one ordinary afternoon — and it means the tool declined to destroy other panes' uncommitted beads, not that your commit failed. The generated files it *does* repair are exactly the ones that have no foreign lines to protect.

**Its positive case is staleness by OMISSION, and that is narrower than it sounds — the common manifest case REFUSES.** The check is line identity, so a stale *version* of a row is a line `HEAD` has never seen and is scored foreign exactly as a peer's new work is. Measured on 2026-08-05 while chasing a real red: the worktree's `franken_lean-gii.26` coverage row was the pre-fill version its author had since completed and committed — HEAD's copy carried eleven populated fields the worktree's lacked and the worktree's carried **none** HEAD lacked, so it was strictly poorer and unambiguously stale — and the sync still refused, `count=1`, repairing nothing. Meanwhile `real_verification_manifest_covers_the_live_tracker` reads the **live** file, so `structure-guard` was red for every pane on `requirement_ids must not be empty` against a `HEAD` that validated clean. **So a refusal is not always benign: on `.beads/issues.jsonl` it protects peers, and on a generated or single-owner file it can leave a repository-wide red the tool was built to prevent.** When it refuses, diff the file against `HEAD` per record and ask which side is *newer* — if no worktree field is newer anywhere, restoring `HEAD`'s copy is a provable restoration rather than an overwrite, and that is the argument to make in the commit or route rather than reaching for the tool a second time.

**Before chasing that, read `real_verification_manifest_covers_the_live_tracker`'s message: it has a SECOND cause and only one of the two is yours to fix.** Both were observed within one hour on 2026-08-05. `requirement_ids must not be empty` is the stale-copy case above. `beads crossed the adoption boundary without coverage rows: [...]` is a peer **mid-filing** — `br` auto-flushes a new bead into the shared worktree tracker the instant it is created, while the bead, its projection and its coverage row land together in one commit some minutes later, so this test is red for every pane throughout that window **by construction** and clears itself. Do not repair it, and do not file against it; the lateness is `franken_lean-xjjr`'s subject. The general rule this instantiates is two sections up — a red channel is a claim with an expiry, and what expires is not *whether* it is red but *which defect* is making it so.

**Measured in both directions before being relied on, in a throwaway repository, because a tool that refuses EVERYTHING is indistinguishable from a working one until you make it succeed.** A worktree copy that is a strict stale subset of `HEAD` is **synced**, exit **0**, emitting an `fln.private-index-worktree-sync/1` receipt naming the head and the paths; a copy carrying one line absent from `HEAD` is **refused**, exit **2**, bytes unchanged. Cell two is what makes cell one worth anything — without it, a tool that blindly overwrote would pass the first cell too. **The exit code discriminates here, unlike most of this file's other refusals** — and the first version of this paragraph said it did not, because its author read `$?` after piping the tool through `head` and reported `head`'s status. That is the trap two sections above, fallen into while quoting it. **Read the exit code from the process.**

**What this does not earn.** Nothing enforces any of it: no hook can see how a commit was assembled, and the gate is a rule in a script one pane wrote. It says nothing about a peer's write landing between the gate and git's own read. Nothing runs the worktree sync for you either — it is one more line in a recipe, which is precisely why it went unrun for two days after it landed. One host, one git version, one instant, class `bounded_model`.
...
| **M4** | `stable_file_facts` `fstat`s each governed file **before and after reading it** | `scripts/evidence.py` → `file changed while being read`; `check.sh` names it `governed_input_mutation_during_initial_hash` | the paths being hashed | churn of a **governed** file during the hash: **8/8 killed**. Churn of an ungoverned file: **8/8 passed**. Lane-level shape (cc_3, bead `franken_lean-build-gate-lane-governed-set-98np` comment 2078, private clone, no gate window): the churn reaches the terminal record as `verdict=internal_fault reason=final_workspace_hash_unavailable`, exit 2 — a distinct FL-INV-07 class from M3's inconclusive/exit 3, so an operator reading exit 2 goes hunting a broken tool when the cause is a file that moved under the reader |
| **M5** | `stable_symlink_facts` — M4's sibling for a governed **symlink**: `lstat` rather than `fstat`, and it hashes the link's **target string**, not the target's bytes | `scripts/evidence.py:2135` → `canonical link changed type`, `symlink changed while being read` | the governed symlinks being hashed | **retargeting a symlink moves the governed root with no file content edited anywhere** — the one write shape M2/M3/M4 all read as "nothing changed". Measured by cc_1 in `76298969`; the row above is that measurement plus the code, and I have re-derived only that the function exists and which refusals it raises |
...
**A probe that says HELD is wrong in the mirror direction, and that is the half this file was asserting rather than doubting.** Bead `franken_lean-gate-lock-producer-optional-o2vz` records why FREE carries no information: the gate lock engages only if the caller volunteers, so an unwrapped lane leaves nothing for any probe to observe. Derived at `8c13c543`, the population is sharper than "no lane acquires it" — `fln-gate.lockfile` was named in exactly **three tracked files**, `.beads/issues.jsonl`, this file, and `ci/VERIFICATION_MANIFEST.jsonl`, every one of them prose or record, and **zero executable surfaces named it at all**. **BOTH HALVES OF THAT ARE NOW FALSE, and the sentence they support with them.** Re-derived on 2026-08-05 at `abedbcf3` by the same method: **five** tracked files, of which **two are executable** — `scripts/lib/gate_lock.sh`, which defines the path, and `crates/fln-conformance/tests/gate_lock_discriminator.rs`. `scripts/check.sh:80` **sources** that library and calls `fln_gate_acquire` at line 97, and thirteen declared lanes source it too. So the premise "the gate engages only if the caller volunteers" no longer describes the full gate: **`check.sh` volunteers, every time.** FREE remains weak for the *other* reason the box above gives — a lane acquires with a waiting `flock -w 2400`, so it can be dispatched and queued while the probe still reads free — but it is no longer weak because nothing acquires. This is `o2vz`'s own producer landing and invalidating the measurement that motivated it, which is why the claim carries its derivation date.

**And the path is an ENVIRONMENT OVERRIDE, which cuts both ways.** `gate_lock.sh:15` reads `FLN_GATE_LOCKFILE="${FLN_GATE_LOCKFILE:-/data/tmp/fln-gate.lockfile}"`. Usefully, that is what lets a lane or the full gate run against a **private** lockfile inside a throwaway clone, so a measurement that would otherwise freeze ten panes freezes nobody. Dangerously, it means the machine-wide gate is **opt-out by a single exported variable**: a pane that sets or inherits `FLN_GATE_LOCKFILE` runs unsynchronised with everyone else while every probe in this section still reads FREE and every rule above still believes it. Nothing warns, and no check in this repository observes it. The one `flock` in `scripts/evidence.py` is an unrelated `fcntl` lock on its own descriptor, identical in the committed blob and the working copy. HELD then fails for the mirror reason: *anything* may take that path. Measured by the sequencer at 2026-07-27T08:25:56Z — a pane's `flock -n /data/tmp/fln-gate.lockfile zsh -f` (pid 2719272, state `S<s`, WCHAN `do_wait`) held the gate for nearly three minutes with **no lane running**. That form is not a probe: it runs a shell under the lock for that shell's entire life, and the probe form is the `-c true` one in the box above. Reproduced by cc_1 on a scratch lockfile at `8c13c543`, same state signature, with a planted non-lane holder: two consecutive `-c true` probes both exit 0, so the probe never holds; under the plant the prescribed probe exits 1.
...
**Narrowness is a property of the lane you are running, never of lanes.** Derived per commit from all 35 scripts in `scripts/e2e/` rather than read off one — 98np R1. **Twenty-two lanes declare a governed set; thirteen declare none at all** and so cannot raise M2/M3/M4 under any write:

| governed paths | lane | relative to `check.sh`'s 68 |
|---|---|---|
| 40 | `contract_handoff.sh` | 2 outside: `scripts/extract/census_materialize.sh`, `…/validate_extern_builtin_census.py` |
| 18 | `bignum_vectors.sh` | 1 outside: `scripts/e2e/marrow_stage0_gauntlet.sh` |
| 18 | `campaign_frameworks.sh` | contained |
| 10 | `attribute_state_census.sh`, `suite_upgrade_no_mock_e2e.sh` | contained |
| 19 | `contract_drift.sh`, `vellum_naming_no_mock_e2e.sh` | 3 outside: **`AGENTS.md`**, `README.md`, the plan |
| 18 | `closure_audit.sh`, `structure_gate.sh` | contained |
| 16 | `cartridge_no_mock_e2e.sh` | 2 outside: `CERTIFICATE_FORMAT.md`, `CARTRIDGE_FORMAT.md` |
...
So "an e2e lane's governed set is narrower than `check.sh`'s" is true of `env_snapshots` and **false of three lanes in three different directions**. Two consequences: a `verdict_schema` lane is killed by any pane touching `scripts/`, which is broader than `check.sh` in that dimension; and the **eleven** zero-governed scripts are protected by **nothing but M1**, so a mid-lane write to their subject is invisible to every other mechanism. **Two numbers in this sentence were wrong at once and each rotted differently, which is why the guard below now binds all three cardinalities in this section rather than one.** It said "twelve", contradicting the table's own `| 0 | the other 12 |` row two lines above it — a count restated in prose beside the table it disagrees with, item 7's shape at a distance of two lines. And it asserted that "no lane takes the gate lock unless its caller wraps the invocation", which was true when `o2vz` was filed and is not true now: re-derived at HEAD while landing `o2vz`'s wiring half (2026-08-24), **22 declared lanes source `scripts/lib/gate_lock.sh` and take the gate themselves**, of 22 — every declared lane self-acquires, so a `flock -n` FREE probe is informative for every lane, and a HELD probe names its holder through `/proc/locks`. The eight lanes the previous figure carried as not taking it were wired in that pass; the guard's anti-degeneracy assertion (which required a non-empty not-taking set) was inverted with them, because a completed migration leaves that set empty by design — the guard now pins the declared emptiness and fails if any future lane stops acquiring without this file moving too.

**That denominator said 22 and that list said nine, and both were wrong for ONE reason — this passage counted `suite_upgrade_candidate_preflight.sh` as a declared lane.** It is not: it emits no `fln.e2e/2` schema at all, which is the definition used two paragraphs above, and it governs by a different idiom entirely (`require_unchanged_root` at eight call sites) — a fact this very section already records as a named allowance. So the same script was excluded from the lane population in one paragraph and silently included in the next, and 13 + 9 = 22 stayed internally consistent while being externally false. Re-derived independently by cc_1 and by cc_3 on 2026-08-05 at `72b98455`, from the scripts rather than from each other: **33 scripts in `scripts/e2e/`, 21 declaring `fln.e2e/2`, 13 sourcing the gate lock, 8 declared lanes not taking it**, and zero scripts that take the gate without being declared lanes. **A count that adds up is not thereby derived** — this one balanced for two days because both of its halves shared one wrong member.

**(The 13/8 gate-lock split in the dated paragraph above is superseded, 2026-08-24: all `21` declared lanes now source `scripts/lib/gate_lock.sh`; the current-state sentence two paragraphs up carries the live figure. The 72b98455 derivation stands as history.)**
...
**And "outside this lane" is not a property of a path — it is a property of a path AND a lane, which is the trap waiting for anyone who measures the freeze rather than derives it.** `98np` R2 asks for a write planted in "a tracked path outside it" for `env_snapshots` and for `check.sh`. The obvious choice is something under `crates/`, and for `env_snapshots` **that is inside**: it governs three crate trees — `crates/fln-core`, `crates/fln-env`, `crates/fln-hash` — among its thirteen entries. `check.sh` is worse in the other direction: five of its entries are **bare directories** (`ci contracts crates tools tribunal`), which between them cover essentially every source file here, so almost nothing under version control is outside it. Derived on 2026-08-05 at `72b98455`; the working choices are `crates/fln-kernel/…` for `env_snapshots` and `README.md` or the plan for `check.sh`. A cell planted at a path the experimenter *believes* is outside measures "voids", records it under the outside column, and produces a well-formed wrong answer of exactly this section's family — **so derive the containment for the specific lane before you plant, and state which path you used.**
...
**The window opens at `br close`, NOT at your commit, and that is earlier than this rule reads.** `br` auto-flushes into the shared `.beads/issues.jsonl` **working copy** the moment it acts, so from that instant the live pair — working tracker against working manifest — has a closed bead and a sparse row, and every pane validating the *working* pair sees a red that is yours. Making the close and the row one **commit** does not close that gap; it only bounds what lands. Measured on 2026-08-05: cc_3 closed `fln-manifest-complete-row-preflight-gap-77xh`, and in the minutes before the row commit, cc_1 diagnosed the resulting `requirement_ids must not be empty` as a live workspace red, routed it to the wrong owner (the row still carried its filer's `owner` field), and lost time on a defect that was **already being repaired**. Nothing was wrong with either pane's work. **So author the row FIRST, or hold the close until the row is ready in the same shell block** — and if you must leave the window open, say so where the swarm reads rather than where you commit, because the red arrives for everyone before the commit that explains it does.

**`blocked` means TWO DIFFERENT THINGS one line apart in the validator's own output, and the one you are reading is almost certainly not the one you think.** Every `validate-verification-manifest` run prints `derived_state_counts` including a `blocked` tally — and that is a **coverage ROW's disposition, derived from the row's `skip` field**, never a bead's lifecycle status. `derived_verification_coverage_state` in `scripts/evidence.py` says so in its own docstring and calls the collision deliberate; nothing in the output does. Measured on 2026-08-05 by cc_1, who read `blocked: 1` a dozen times across a day while investigating bead statuses: **beads at status `blocked` = 0**, rows with `skip: "blocked"` = **1**, and that row's bead (`fln-d18-product-half-rgsg`) is **open**. Two panes have now lost time to it from opposite directions — one chasing a bead that was not blocked, one filing a bead about the word itself (`franken_lean-px33`, folded into `franken_lean-shlw`). **A `blocked` in `derived_state_counts` is a row; a `blocked` from `br` is a bead. Check which producer emitted the word before acting on it.**
...
**So filing a bead is three artifacts in one commit, not one** — the bead, the regenerated ownership projection (gotcha 1 below, since any new bead stales it), and a coverage row. The row for an open bead is legitimately **sparse**: every evidence array empty, the notes carrying the measurement and what it does not establish, because no repair is being claimed yet. `franken_lean-build-gate-lane-governed-set-98np`'s row is the model and says so in its own text — "This row exists because filing a bead obliges one; it records an observation and an obligation, and claims no repair." (It cited `franken_lean-evidence-fields-never-resolved-bs5o` until that bead closed and its row stopped being sparse; an exemplar of an *open* bead's row is falsified by its own subject closing, so this citation expires the same way and the replacement was chosen for saying it in its own text rather than merely being empty.)
...
The law is **satisfiable** for all 42 — a comment added today to a bead closed last week has `created_at` > `closed_at`, and 36 of the 42 already carry one — so the honest repair is to bind the disclosed exempt population to the measured one, **equality in both directions**, plus the conservation identity `exempt + bound == complete` as the anti-vacuity guard. Then 42 silent additions become 42 deliberate, disclosed ones: a new exempt row pushes 105 to 106 and its author must either bind the row or raise the number under review. Equality both ways is right here and one-way-plus-floor is not — that shape is for a declared remainder of *permitted violations*, which shrinks as people repair it; this is a disclosure of a *measured population*, which does not. **The guard is now built, and it DEPARTS from the prescription in the two sentences above — the departure is the finding.** It is `crates/fln-conformance/tests/closure_binding_population.rs`, and it plants no second copy of the predicate: it *runs* `validate-verification-manifest` and reads that command's own `closure_exempt_rows`, `closure_bound_rows` and `derived_state_counts.complete`. That is the reason this passage gave for wanting the guard inside `scripts/evidence.py`, satisfied without living there. What the prescription got wrong is the **typing**. Measured for this bead on three dates: `exempt` 105 → 105 → 105, unmoved, while `bound` went 8 → 18 → 108 and `complete` 113 → 123 → 213 — so **equality on `exempt` plus the identity would have been GREEN through a hundred-row drift in the two figures stated beside it**. `exempt + bound == complete` is a *partition*: every complete row is exactly one of the two, so it holds by construction of the classifier and can fail only when the classifier breaks, never when a population moves. It is a check on the producer's internal consistency, and this passage sold it as the anti-vacuity guard for the disclosure. The three are therefore typed differently. `exempt` by **equality in both directions**, exactly as prescribed and for the reason prescribed: it does not shrink on repair and it grows silently into those 42 slots. `bound` and `complete` by **floors**, because both rise on every close by any pane, so an equality gate there is a wall that reddens exactly the commits recording good work — the cry-wolf failure this file already measured when a census drifted 26 → 27 → 28 against a live population that never moved; a *fall*, which means a close vanished or the classifier lost rows, still fails. The anti-vacuity that does the work is explicit instead of inherited: a zeroed measurement satisfies every equality, both floors **and** the partition, a zeroed measurement is refused by name, and `a_zeroed_measurement_is_refused_as_a_broken_scan` plants exactly that on injected inputs rather than waiting for the day the scan breaks. It judges **HEAD** rather than the working copy, because in this shared checkout every pane's `br` auto-flushes into the working tracker and a peer's `blocked` status made the validator refuse outright on 2026-08-04 while HEAD validated clean — a guard reading the working copy is red for a cause its own author cannot fix. `the_closure_binding_disclosure_still_describes_this_tree` runs under plain `cargo test`.

```text
closure-binding population disclosed by 3s8w: exempt=105 (equality, both directions) bound-floor=108 complete-floor=213
```

**What this does not earn.** It holds the three numbers to the tree; it holds no exempt row to its closure. The 105 remain exempt by date and uninspected, and whether a given row was authored for the closure it is filed under is a semantic question no count answers — this guard never reads a row's prose. The floors admit unbounded growth in `bound` and `complete` by design, so they detect loss and not overclaim. Measured at `2ce4d95c`, one host, one commit, class `bounded_model`.
...
rch-tracker-population: mentions=16 non-reads=2 reads=14
rch-tracker-reads: crates/fln-conformance/src/naming.rs crates/fln-conformance/src/ownership.rs crates/fln-conformance/tests/agents_enforcement_census.rs crates/fln-conformance/tests/campaign_owner_matrix.rs crates/fln-conformance/tests/ci_execution_join.rs crates/fln-conformance/tests/closure_binding_population.rs crates/fln-conformance/tests/commit_anchor_reachability.rs crates/fln-conformance/tests/done_but_open.rs crates/fln-conformance/tests/evidence_finalization.rs crates/fln-conformance/tests/w4_macro_join.rs crates/fln-syntax/tests/golden_vellum.rs tools/structure-guard/kernel-ownership-publisher/src/main.rs tools/structure-guard/src/bin/fln-frontier-guard.rs tools/structure-guard/tests/real_workspace.rs
rch-tracker-non-reads: crates/fln-conformance/tests/vellum_surface_inventory.rs crates/fln-env/src/extensions.rs
git-shelling-suites: crates/fln-conformance/src/tree_identity.rs crates/fln-conformance/tests/artifact_referent_census.rs crates/fln-conformance/tests/closure_binding_population.rs crates/fln-conformance/tests/commit_anchor_reachability.rs crates/fln-conformance/tests/done_but_open.rs crates/fln-conformance/tests/evidence_field_citations.rs crates/fln-conformance/tests/evidence_finalization.rs crates/fln-conformance/tests/kernel_contract.rs crates/fln-conformance/tests/kernel_replay.rs crates/fln-conformance/tests/mandated_mutants.rs crates/fln-conformance/tests/mutation_kill_ledger_model.rs crates/fln-lake/src/lib.rs crates/fln-syntax/tests/golden_vellum.rs tools/structure-guard/src/bin/fln-agent-anchor.rs tools/structure-guard/tests/agent_anchor_cli.rs
git-shelling-population: files=15
```

**The same exclusion list drops `fuzz/corpus/`, and that one is INERT TODAY — recorded now so it is not discovered by a green** (`fln-y0f7` R4). Measured at `8fb861a9`: the 41 `exclude_patterns` contain `fuzz/corpus/` and `fuzz/corpus/**`, this repository has **no `fuzz/` directory at all**, and its golden corpora live at `crates/fln-syntax/tests/corpus` and `crates/fln-verdict/tests/corpus` — paths the pattern does not match. So the exclusion currently removes nothing. **It becomes live the moment a §18 fuzzing rig lands at the conventional `fuzz/` path**, and the failure it produces then is the worst kind: a corpus that is silently empty on every worker, and **an empty corpus passes**. That is the false-GREEN direction, the one `k60n` was release-blocking for, and unlike the tracker case no reader would fail loudly — a fuzz harness given nothing to chew simply finds nothing. `the_rch_fuzz_corpus_exclusion_is_still_inert` binds the claim in both directions and fires the day `fuzz/` appears, because a guard whose population is empty is decorative until something plants a member in it.

**One property, two environments, and the affected set is DERIVED — `46pw`.** A suite that shells out to `git` needs a **real `.git` directory**, and this repository has *two* common environments that do not have one: a **linked worktree**, where `.git` is a `gitdir:` pointer file (`franken_lean-worktree-gitdir-refusal-hugg`), and an **rch clean-overlay worker**, which syncs the source tree with **no `.git` at all**. Both are actively recommended — worktrees to dodge a dirty shared tree, `rch` to dodge local load — so *the better a guard follows this project's derive-your-scope doctrine, the less able it is to run in either*. `git-shelling-suites` above is that set, **derived per commit** rather than hand-listed, which is `hugg`'s own recorded open gap ("the affected-surface list is written down, not derived, so a NEW lane that starts refusing would go unnamed"). `the_git_shelling_population_is_derived_and_reconciled` binds it in both directions, floors it, and reconciles the primary spelling against an independent one — three extractors keyed on a single spelling produced a false zero on 2026-08-04 alone. **The failure these suites give you names neither environment**: `fatal: not a git repository`, which reads as a broken checkout rather than as an environment that cannot host the test.
...
### The worker may lack a component of the pin — sealed admission separates fmt before the ambiguous exit
...
`rust-toolchain.toml` declares `components = ["rustfmt", "clippy", "miri", "rust-src"]`. Those are components **of the pin**, not a second pin — a machine holding `nightly-2026-07-13` without `clippy` does not have this repository's toolchain. The first version of the sealed compiler check read only `channel`, so the right nightly with a missing component passed admission. `parse_rust_lock` now derives both the channel and the component array, and a malformed or empty declaration is the typed setup fault `sealed_compiler_components_unreadable`, never an empty component set.
...
**The measured 2×2 that exposed the defect.** These are the raw Cargo exits; the final column states the current gate behavior after sealed admission:

| gate | real code finding | component absent | `check.sh` registers | gate behavior |
|---|---|---|---|---|
...
| `cargo fmt --check` | **1** | **1** | `--semantic-failure-exit 1` | **yes, before spawn** — sealed admission requires the declared pinned `rustfmt` component's `cargo-fmt` and `rustfmt` executables; absence is `sealed_compiler_component_absent`, while exit 1 is consumed only after that precondition holds |

The clippy margin remains load-bearing: **do not "simplify" 101 to 1**, because that single edit converts an absent component into a semantic rejection. The fmt repair cannot come from its exit code because both meanings are 1. `prepare_sealed_cargo` therefore maps the exact `cargo fmt` command to component `rustfmt`, requires that component in the parsed pin, and checks both `cargo-fmt` (Cargo's subcommand surface) and `rustfmt` (the formatter it drives) under the already identity-verified pinned toolchain root before the target process exists. Missing declaration and missing executables are separate typed setup faults; neither is a formatting verdict. This is the `franken_lean-fmt-gate-env-fault-as-finding-u4j7` repair, not a claim that Cargo itself changed its exit taxonomy.

The executable controls are part of the mechanism. `scripts/evidence.py self-test` constructs one pinned-root view with the real pinned `rustc` and `cargo` but neither fmt executable: `sealed_fmt_component_absent` must produce `internal_fault/sealed_compiler_component_absent` with `execution_start_ns=null`. Separate cells remove `rustfmt` from the declared array and make that array unreadable; both must refuse before execution under their own reason codes. It then installs `cargo-fmt`, `rustfmt`, and one execution sentinel in the same root: `sealed_fmt_component_recovery` must dispatch through real Cargo, execute the sentinel, and pass. A preflight that always refuses satisfies the negative cells and fails the recovery. Four metadata mutants separately prove that an undeclared envelope cannot list `rustfmt`, an admitted envelope cannot omit it, and the executable names and paths cannot drift.
...
`the_pin_declares_components_and_sealed_admission_separates_fmt_absence` derives the component list from `rust-toolchain.toml` rather than transcribing it and binds this section to the typed refusals plus the recovery. `a_clippy_stage_that_registers_the_component_absent_exit_is_caught` preserves the independent clippy margin; `dropping_the_fmt_component_declaration_cells_is_caught` binds the pin parser; `dropping_the_fmt_component_recovery_cell_is_caught` prevents the negative-only wall from reading as a repair; and `dropping_the_fmt_component_validator_mutants_is_caught` binds the envelope facts to their reason semantics.

**What this does not earn.** The RCH figures are one host at one instant, class `bounded_model`, and the capability surface is outside this repository — a version bump changes it silently. The command-to-component admission table currently contains only `cargo fmt` → `rustfmt` (`cargo-fmt` plus `rustfmt`); clippy remains separated by its measured exit taxonomy, and neither `miri` nor `rust-src` is preflighted for commands that do not require it. Executable presence proves the named command can be admitted, not that every file in the installed component is intact.
...
4. **Writing a new `fln.e2e/2` lane**: model on `scripts/e2e/closure_audit.sh`; every `--wait-ms` for the process-identity guards is capped at **30000** (a larger value makes the guard raise instantly and the lane SIGKILLs its own runner with a bare "Killed"); every scenario MUST be registered with its exact ordered step list in `E2E_STEP_ORDERS` at the top of `scripts/evidence.py`; register the script in `scripts/check.sh` (INPUT_PATHS + shellcheck stage) and in a reachable workflow that can satisfy the lane's dependencies (`.github/workflows/ci.yml` for the ordinary case; the pin-bearing scheduled/on-demand workflow for a Reference-dependent lane). New `ci.yml` e2e steps must also join the verify-step's `expected_roots` set and `specs` tuple — the roots set is closed. **Registration and configured dispatch prove only that a run can occur, never that one did occur:** per-run evidence requires the workflow to retain and validate that run's committed bundle. Expected-fail cargo steps use `--semantic-failure-exit 101` and must grep BOTH `.out` and `.err` captures for the intended reason (libtest panics print to stderr under `--nocapture`).
5. **While an e2e lane holds the gate lock, change NOTHING inside the repository** — see [The Build Gate](#the-build-gate--while-a-lane-runs-the-repository-is-frozen) above for the mechanism and the full rule. Stated here only because this is the list people read: `INPUT_PATHS` (`scripts/check.sh:197`) governs `governed_inputs_changed` and is a *path list*; the freeze that ends a lane is `repository_state()` and is *path-agnostic*. **During a lane, every tracked file is effectively a governed input.** Reading only the path list is how "I checked, my file wasn't on it" kills a run — item 7's defect family one floor down, two artifacts with neither naming the other. **This entry used to add "…and does not include `AGENTS.md`", which was true of `check.sh` and read by three people as true of lanes.** It is not: `scripts/e2e/vellum_naming_no_mock_e2e.sh` governs `AGENTS.md`, `README.md` and the plan, so for that lane an *uncommitted* edit to this file is enough. There is no single path list to check — each lane declares its own; the per-lane table is in [The Build Gate](#the-build-gate--while-a-lane-runs-the-repository-is-frozen) and is derived per commit, so **read it there rather than restating a count here.** **The clause that stood here — "and twelve of the twenty-five declare none" — was wrong in both numbers, and restating a derived cardinality away from the table that derives it is how it got that way** (`franken_lean-build-gate-lane-governed-set-98np`, R4). Re-derived at `f042f0dc`: `scripts/e2e/` holds **31** scripts, **20** declare a governed set and **11** declare none — and the split is exactly the `fln.e2e/2` boundary, so **every declared lane had a governed set** and the eleven with none were the eleven that are not declared lanes. **That rule survived eight hours, and I got the correction wrong TWICE before measuring it.** At `a9a42f5f` two new `fln.e2e/2` lanes landed and the derivation reported both as governing nothing. The guard's **reconciliation signal** — a derived zero contradicted by an independent count of governance references — refused that, twice, and was right both times. **First**: `contract_drift.sh` governs **19** paths declared by an `INPUT_PATHS=(` **indented inside a function**, which the extractor's line-start rule silently missed — the `AP6_INPUT_PATHS` false zero again, in a new spelling, caught by the check the first one bought. The extractor now admits leading whitespace. **Second**: I then wrote that `suite_upgrade_candidate_preflight.sh` was a stub whose governance references were prose. They are code — `require_unchanged_root` at **eight call sites**. That lane governs by a **different idiom** and declares no array at all. So the honest statement is narrower than the rule it replaces: **every declared lane governs something; the table covers the ones that say so with an `INPUT_PATHS` array**, and one lane governs outside it, carried as a named allowance rather than a predicate so it cannot silence a real miss. Three derived zeros in one repair, three extractor gaps, and the signal caught every one — an extractor keyed on a single idiom will keep doing this, and the reconciliation is what makes it survivable.
...
7. **The recurring defect: evidence must be produced where the claim is made.** Stated once, generally, because it has now been found eleven times and every single time by somebody *reading carefully* rather than by a check:
   ...
   **A standing habit even after `vdi4` closed the repository-wide half:** whenever you re-derive a measurement, record the hash you re-derived it **at**. A fresh anchor costs one `git rev-parse` and is the only thing stopping the next reader from being stranded the way this section's own examples now strand one. The retention guard says whether a historical token is still present and how it resolves; it does not recreate the measurement the token was meant to identify.
   ...
   **This file's own enforcement claims are now counted, by a producer in the repository** (bead `franken_lean-pfei` R1). AGENTS.md is the densest source of unbound enforcement claims here, and four of its claims were measured false in two days — so the population is derived per commit rather than described:
   ...
   ```text
   enforcement-census: live=35 bound=19 unbound=16 catalogued=13 reviewed=16
   ```
   ...
   **Read `bound` as "names a candidate referent in the same sentence" — never as "verified".** PFEI R2 now resolves four addressable kinds: line citations; test functions, including whether they run; lane/workflow paths, including `scripts/check.sh` registration, CI execution, and top-level trigger reachability; and bead IDs in `.beads/issues.jsonl`. The Python census is the sole sentence extractor and emits lane/workflow/bead referents to the Rust judge, so there is no second enforcement-verb regex to drift. A named source file, source site, or test can still exist while failing to prove the sentence's **argument**; R2 closes denotation and reachability, not semantic truth.

   **R3 makes the remainder explicit without laundering it into proof.** Every still-unbound live claim matches exactly one reviewed row below, and every row matches exactly one still-unbound claim; IDs, needles, reasons, count, and a ceiling of 16 are part of the gate. The reasons remain prose. They state why no repository-local sentence-to-producer join is currently honest; the gate establishes the bijection, not those reasons' truth. The independent R5 floor is `live >= 35`, so softening a sentence, lowering the disclosure, and retiring its row exits 2 before the coordinated edit can become a hollow green.

   > reviewed-unwalked PFEI-U01 :: documentation CI rejects wording stronger :: the claim-matrix mechanism is described here, but this sentence-to-matrix join remains unwatched
   > reviewed-unwalked PFEI-U02 :: caller-chosen type parameter :: executable fixture and production-path evidence exists for the generic-return rule, but this sentence names no producer
   > reviewed-unwalked PFEI-U03 :: CI counts release-compiled authority and walks the graph :: release-authority and graph checks exist elsewhere; this broad conjunction has no single sentence-local referent
   > reviewed-unwalked PFEI-U04 :: CI rejects an unledgered site :: the unsafe-site ledger guard exists, but this sentence names neither its test nor source site
   > reviewed-unwalked PFEI-U05 :: refuses a vacuous exclusion :: generated-output anti-vacuity is exercised elsewhere, while this sentence names no producer
   > reviewed-unwalked PFEI-U06 :: records *that* the evidence surface refuses a linked worktree :: a metatextual statement about this document rather than an addressable mechanism claim
   > reviewed-unwalked PFEI-U07 :: admits a still tree and refuses a moving one :: an external RCH measurement with its control, not a repository-local producer named on this line
   > reviewed-unwalked PFEI-U08 :: rch refuses any project path outside :: external RCH policy; no producer for it lives in this repository
   > reviewed-unwalked PFEI-U09 :: kernel divergence blocks release :: release policy backed by several suites, but the sentence-to-suite join remains unwatched
   > reviewed-unwalked PFEI-U10 :: a surviving critical mutant blocks the gate :: campaign policy and on-demand evidence exist, but no unique killer is named in this sentence
   > reviewed-unwalked PFEI-U11 :: M4 only refuses a file that moves :: scripts/check.sh implements M4, but the explanatory sentence names no addressable producer
   > reviewed-unwalked PFEI-U12 :: Nothing holds any of it to the code :: deliberate disclaimer that no mechanism covers the preceding measurements
   > reviewed-unwalked PFEI-U13 :: stale projection turns `cargo test` red :: a projection guard exists, but this sentence does not name it and the join remains unwatched
   > reviewed-unwalked PFEI-U14 :: So it is enforced rather than remembered :: the producer is described on the preceding physical line, which the sentence-local scan deliberately does not merge
   > reviewed-unwalked PFEI-U15 :: leftover candidate fails the gate typed :: generated-candidate policy has a guard elsewhere, but this sentence names no referent
   > reviewed-unwalked PFEI-U16 :: validator enforces count conservation :: the validator mechanism exists, but the sentence names no test or source site
   ...
   **The 27 → 28 movement on 2026-07-27 is disclosed here with its reason, because the reason exposed a limit of the scan rather than of the sentence.** The new member was the coverage-row obligation added to the judgement-row section, and it named its producer in plain English — the verification-coverage-guard living in the extensionless pre-commit hook. The scan could not see it: `test-fn` required a closing backtick immediately after the token, while `source-file` recognised only seven file **extensions**. The count was raised rather than the sentence softened. **PFEI R2/R3 now repairs that measured limitation:** `source-file` recognises the exact extensionless `scripts/git-hooks/pre-commit` path, moving the current split from bound/unbound 16/17 to 17/16 without changing `live`. The history remains here because it is the control explaining why an extensionless executable is explicit rather than accepted by a broad path guess.
   ...
   **The 32 → 33 movement is the R3 case this census exists to make cheap, and it raises `unbound` rather than `bound` — 16 → 17.** The new member is the RCH staleness result above: a sentence asserting that a size-and-mtime-preserving change is not transferred. It names no producer in this repository **and cannot**, because the producer is `rch`, which lives outside the tree — the same limit already recorded for the UBS version and the worker capability surface. That is precisely the "genuinely reviewed-not-walked" category R3 legitimises, so the honest act is to raise the number under review and say so here. Rewording the sentence into "appears to" would have gone green and destroyed the only durable record of a measured mechanism, which is pfei R5 exactly.

   **The R3 landing changes the split, not that history:** the extensionless-hook repair above moves one member from unbound to bound, while the sixteen-row registry declares the exact remainder. A new unbound sentence needs a new unique row and disclosure movement; a repaired or newly bound sentence makes its old row stale; a seventeenth row crosses the code ceiling. The floor is independent of all three coordinated surfaces, which is the R5 cell the earlier census lacked.
...
```text
cite tools/structure-guard/src/checks.rs:1667 :: constitutional prohibition
cite tools/structure-guard/src/checks.rs:1918 :: let mut actual_edges
cite tools/structure-guard/src/checks.rs:2024 :: (CrateKind::Tool, _)
cite tools/structure-guard/src/checks.rs:2075 :: code: "FLN-STRUCT-008"
cite tools/structure-guard/src/boundary_api.rs:109 :: fields.iter().any(|f| f.is_empty())
cite tools/structure-guard/tests/real_workspace.rs:50 :: fn real_workspace_is_structurally_clean
...
cite scripts/check.sh:80 :: . "$REPO/scripts/lib/gate_lock.sh"
cite scripts/check.sh:197 :: INPUT_PATHS=(
cite scripts/lib/gate_lock.sh:15 :: FLN_GATE_LOCKFILE=
cite scripts/e2e/vellum_naming_no_mock_e2e.sh:93 :: INPUT_PATHS=(
cite scripts/evidence.py:2135 :: def stable_symlink_facts
cite scripts/evidence.py:18535 :: def repository_state
```
