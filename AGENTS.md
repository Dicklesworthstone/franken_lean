...
The standing advice, correct and unchanged, is: when another pane's in-flight edit reddens the shared tree, verify in a **pinned git worktree at HEAD plus your patch** rather than waiting or touching their files. That is sound for **cargo suites** — `cargo test`, `clippy`, `fmt`, individual test targets — and it is how the guard in `9a6f7941` was verified.
...
|---|---|
| `cargo test` / `clippy` / `fmt`, any target | **yes** — this is the sanctioned escape hatch |
