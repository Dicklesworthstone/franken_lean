...
**The mechanism, measured rather than described.** **Four** checks can end a lane, not one, and which of them you are subject to decides which mid-lane action is fatal. Measured at `3407ca10` against the *committed* `scripts/evidence.py` (blob `79172e7f`), every cell with a positive control, in a purpose-built real repository — bead `franken_lean-build-gate-lane-governed-set-98np`, comment `1367`:
...
2. "…and `AGENTS.md` is outside `INPUT_PATHS`, so it is safe mid-lane" — the half of that sentence about `INPUT_PATHS` was *correct* and the conclusion still wrong, which is what makes it worth keeping. `24b16eeb` was an `AGENTS.md` **commit** at 21:30:59, and it killed the rerun at 21:31:10 through M1's `HEAD` half, which no path list governs. An uncommitted `AGENTS.md` edit would have survived all four mechanisms — a fact about the mechanisms, and **not** a licence to edit mid-lane, since the next thing anyone does with an edit is commit it. **The section you are reading replaces the one that did it.**
...
**What this section still does not earn — stated because it is exactly the defect family of item 7 below, sitting on the doctrine that costs the most lanes.** Nothing holds any of it to the code. The table above is a **measurement written down**, not a derived one: the four mechanisms were found by reading `scripts/evidence.py`, `scripts/check.sh` and one lane script, so a *fifth* could exist and nothing would say so, and each lane's `INPUT_PATHS` is transcribed nowhere and verified by nobody. Compare `the_evidence_surface_refuses_a_gitdir_pointer_root`, which does hold the green-bar table to the code one section up. The repairs are open as `franken_lean-build-gate-lane-governed-set-98np` R1 (derive every lane's governed set mechanically instead of reading it) and R4 (a test that fails when a lane's governed set moves without this section moving, in **both** directions, with a planted decoy proving the scan is not vacuous). Until R4 lands, treat this table the way you would treat any claim whose evidence is prose: **re-measure before relying on an edge of it**, and record the commit you measured at.
...
7. **The recurring defect: evidence must be produced where the claim is made.** Stated once, generally, because it has now been found ten times and every single time by somebody *reading carefully* rather than by a check:
   ...
   The nine, with the join that was unwatched — and, more usefully, whether anything would catch a recurrence:
   ...
   | `fln-history-rewrite-evidence-anchor-reachability-vdi4` | a recorded verification and the commit it names, after a history rewrite silently changed every hash | **no** — nothing watches it yet, and no technique above could. Measured at `14bbbe7f`: **166 of 411** distinct commit anchors cited across beads, the verification manifest, AGENTS.md, README, the plan and tracked source are unreachable from `main` after the 2026-07-25 `filter-branch`. They did not become *wrong*; they became **unverifiable**, which is worse here, because every join check above compares a claim against evidence that is still addressable and this changed the address space under every claim at once **without touching a single claim**. Even a naive existence check passes today: the objects survive locally in `refs/original/refs/heads/main` and the tag `pre-filter-branch-backup`, so it is a reader of `main` alone — or this clone after a `gc`, or any fresh clone — who is stranded. The sharpest case is inside the mechanism built to prevent this class: three of the four commits the Witness matrix's own evidence prose cites (`10c9a2e3`, `86035037`, `a368ea0b`) are unreachable, and the Build Gate section above cites two more (`24b16eeb`, `bb561892`) in the passage that teaches why anchoring matters. A guard is cheap and unwritten; it would need a declared allowance that shrinks in both directions, or it can never go green and becomes a gate people bypass. The bead (cod_3) owns the mechanism and states the trap this section would otherwise walk into: object existence is **not** the test, because `refs/original` makes every dead anchor resolve locally — reachability from `main` is |

   **Where the luck is still load-bearing.** Eight of the ten now fail on recurrence; one is half-open; and one has nothing watching it at all. `kl4h`'s movement is mechanised but the contradiction's `Acknowledged` status is not. `uagk` closed its second half: the marker→kill join is now watched per commit by a receipt that expires when either the mutated site or a killer body moves, and the campaign behind it was itself attacked with four planted mutants before being believed. Its cadence half is now half-closed and worth reading for which half: the retention check forces a re-run exactly when the code changes, a weekly workflow dispatches it when the code does *not*, and the receipt's class token is derived from that dispatcher in both directions so neither can move alone. What is still missing is the **observation that a run occurred** — nothing inside the repo can tell a cron that fired from one GitHub quietly disabled, so the token attests a configured cadence, never a kept one, and each run is still one measurement at one commit on one host. `vdi4` is fully open: 166 anchors are unverifiable from `main` today and nothing would notice a hundred more. Everywhere else the mechanism, not the reviewer, is doing the work. Ten instances found by attentive reading is not a mechanism; it is luck with good people, and luck does not survive a context restart. `hugg` is the sharpest argument for that sentence: the practice it invalidated had been corrected by broadcast three times in one day, and broadcasts do not survive a pane restart, which is why the correction is in this file and held there by a test.

   **The two newest rows differ in a way worth keeping.** `bkw6` is a claim whose referent **never existed**; `vdi4` is a claim whose referent **existed and was destroyed wholesale by a tool**. Both defeat the same technique for the same reason — there is nothing addressable to compare a claim against — and neither is reachable by reading one artifact more carefully. The generalisable move is the one `bkw6` used: when the far end is empty or gone, bind the claim to the **cardinality** of what it asserts, and let the number fail in both directions. `hugg` adds a third variant: a referent that **exists and is addressable, but was never reached** — the tooling refused before it ran, and said so in words that named something else. Where a claim rests on a run, check that the run *happened*, not merely that the command was issued and the exit code looked right.

   **A standing habit until `vdi4` is closed:** whenever you re-derive a measurement, record the hash you re-derived it **at**. A fresh anchor costs one `git rev-parse` and is the only thing stopping the next reader from being stranded the way this section's own examples now strand one. Re-anchoring on touch will not converge by itself — it is what keeps the number from growing while nobody is looking.
