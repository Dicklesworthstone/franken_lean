...
- **B4 — Deterministic parallel elaboration.** Declaration-granular dataflow parallelism with speculative execution and deterministic merge: results are schedule-independent by construction (FL-INV-01), tested at {1, 8, 32} threads on every commit — **today that matrix's input is the Prelude; the corpus-scale matrix does not exist yet** (bead `franken_lean-2ki4`).
...
- **Determinism closure.** Thread counts {1, 8, 32} per commit — **over the Prelude**; the corpus differential runs at a single size-derived width and no run compares stream digests across widths, so corpus schedule-independence is inferred, not measured (beads `fln-8zsq`, `franken_lean-2ki4`). Bit-identical artifacts across the certified platform matrix under `--reproducible`; release binaries built twice in isolated builders and compared; the stdlib double-elaboration fixpoint.
...
7. **The recurring defect: evidence must be produced where the claim is made.** Stated once, generally, because it has now been found six times in four subsystems and every single time by somebody *reading carefully* rather than by a check:
   ...
   | `fln-8zsq` | a census and the claim class it supports | **mechanism** — `corpus_census_keeps_disclosing_its_claim_class` reads the source and fails if the SUMMARY row drops its inline qualifier, if the CLAIM-CLASS row loses its `means=`, if the module doc stops scoping the matrix to the Prelude, or if the class strengthens to `measured` while the width is still the size heuristic. It had to be **source**-level: the census is printed by an `#[ignore]`d test, so a stdout grep sees silence, not a missing disclosure |
   ...
   **What this does not earn:** mechanising a disclosure does not upgrade the claim it discloses. Corpus schedule-independence remains **inferred, not measured**. The real evidence is a `{1, 8, 32}` corpus run comparing stream digests, with the width pinned explicitly instead of size-derived — a separate nightly lane, still unbuilt.
