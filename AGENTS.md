...
7. **The recurring defect in this program is that the evidence is not where the claim says it is.** Six instances found so far, in six subsystems, by six different routes — which is why it is written down here rather than rediscovered a seventh time. The claim is always locally consistent; the gap is in the *join* between the claim and its evidence, so no single artifact reads as wrong:
   - `fln-8zsq` — the matrix's evidence covered the Prelude while the census it cited ran over the corpus at one width. **Measured at a different scope than the claim.**
   - `franken_lean-pnav` — shape-only digest assertions delegating to a lane nobody had checked still existed. **Delegated to evidence that may not exist.**
   - `franken_lean-4o3n` — two engines agreeing on a number neither measured in the configuration it runs in. **Agreement about a label, not a resource.**
   - `franken_lean-parity-ledger-l2-definition-split-kl4h` — `L2` means "gated corpus passes" in the plan and "the pinned binary produced the value" in `ci/PARITY_LEDGER.txt`. **The term means something else where the reader looks it up**, and this one is the worst of the family because each document is internally consistent, so it is invisible from inside either.
   - `fln-parity-ledger-l2-pinned-source-qydn` — rows earned by reading upstream's *source* rather than asking the pinned *binary*. **The oracle was guessed at instead of queried**: D8 makes the Reference the differential oracle, and a rig that reads its source has silently replaced it with our reading of it. It stays green until source and built binary disagree — exactly the divergence class the Tribunal exists to catch, arriving where nothing is watching.
   - `franken_lean-ext-observable-fixture-drift-gap-vqnu` — 28 rows resting on a 533-record oracle capture that nothing re-derives. **The evidence was true once and nothing re-checks it.**

   The practical rule: when you write or read an evidence claim, ask *where was this measured, and is that where the claim is made?* If the answer needs two artifacts to state, the join between them needs a check, because nobody reading either one will see the gap.
8. **Identity is taken from the ENCODING, never from the RENDERING.** Renderings are lossy *by design* — that is what makes them readable — so the moment one is hashed, compared for identity, used as a map key, or matched by a build gate, the loss becomes a collision. Four instances in four subsystems, each found separately, each the same sentence:
   - `franken_lean-f6br` — the ArtifactIncomplete census witness digested `Name::to_display_string`, which joins components with `.` **without escaping** and renders a numeric component identically to a string one. Distinct names collided and the witness stopped discriminating. Fixed by binding the *structural* `Name` via `CanonWriter`/`Name::write_body`.
   - `bf9ef450` — `intern.rs`'s `identity_digest` reused that same display form as an intern key.
   - `franken_lean-oof9` — a structure-guard rule keyed on the bare method name `add_decl` cannot separate `fln_env::Environment::add_decl` (one argument) from `fln_hash::LogicalRootBuilder::add_decl` (two), and both appear in one scanned file, so crate-scoping does not rescue it. **Not every instance is a digest**: any projection used as an identity qualifies.
   - `franken_lean-hv9m` — the *same function* as `bf9ef450`, fixed in two halves a day apart: the `Name` half moved to the canonical encoding and the `Level` and `FVar`/`MVar` halves were left on `format!("{:?}")`, with `Canonical for Level` already reachable in `fln-hash`.

   Two questions find these, and the second is the one that is usually skipped. **(a) Can two different values produce the same bytes?** — any variable-length field written without a length, any sum type without a discriminant, any separator legal inside the fields it separates. **(b) Is this projection a CONTRACT, or a rendering that happens to be injective today?** `Level`'s `Debug` answers (a) cleanly — it is genuinely injective — and is still wrong, because its own doc pins it to the *derived* rendering, so a field reorder silently moves every preimage. A renderer that discriminates by luck is a defect waiting for a refactor and **no collision test can see it**.

   Two consequences for how you work. When you fix one, **sweep every sibling call site in the same function before closing** — `bf9ef450` and `hv9m` are one defect fixed twice because that sweep was not done. And test the **encoder**, not discrimination: recompute the preimage independently and assert the digest agrees. A discrimination test passes identically before and after the fix, proves nothing, and quietly certifies the defect.

   Severity varies and should be stated, not assumed: a witness digest *is* the answer, so a collision is a wrong answer (soundness); an intern key is a bucket hint confirmed by full equality, so a collision only blinds `bucket_misses`; a build-gate rule fails loudly at review. Say which kind you have. The mechanical search is every `to_display_string` call site not inside a `Display`/`Debug` impl or a diagnostic string, plus every hand-rolled `hasher.update(..)` sequence that bypasses `CanonWriter`. `fln-kernel`, `fln-olean` and `fln-elab` are unswept.
