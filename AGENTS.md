...
7. **The recurring defect: evidence must be produced where the claim is made.** Stated once, generally, because it has now been found six times in four subsystems and every single time by somebody *reading carefully* rather than by a check:
   ...
   | `fln-8zsq` | a census and the claim class it supports | **nothing** — `kernel_replay.rs` *prints* `CLAIM-CLASS: schedule_independence=inferred_not_measured`, and no test and no lane greps for it. Delete the line and every gate stays green |

   **Where the luck is still load-bearing.** Four of the six would now fail on recurrence; one fails only if a definition moves; one is a disclosure that nothing checks. So `fln-8zsq`'s class — *a census that discloses its own limits in prose nobody verifies* — is the open hole, and `kl4h`'s standing state is the half-open one. Everywhere else the mechanism, not the reviewer, is now doing the work. Six instances found by attentive reading is not a mechanism; it is luck with good people, and luck does not survive a context restart.
