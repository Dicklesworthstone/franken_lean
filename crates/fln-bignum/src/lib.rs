//! **fln-bignum** — kernel-grade arbitrary-precision natural and integer
//! arithmetic — the owned replacement for GMP under every literal and runtime
//! bignum primitive (plan §8.4, §21; bead franken_lean-npl).
//!
...
//! borrows that representation without allocation; [`int::BigIntView`] adds
//! only the runtime `mpz` sign. Marrow binds both views to the lifetime of the
//! ABI object. Arithmetic therefore reads ABI operands in place and allocates
//! only results; the runtime-private mpz layout remains pinned by the G0-1/C4
//! evidence rather than by the public `lean.h` tables.
