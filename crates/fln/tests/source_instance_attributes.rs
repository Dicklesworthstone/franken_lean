...
fn standalone_attributes_drive_the_checked_example() {
    ...
    let checked = base.check_source_files(
        &[include_bytes!("../../../examples/native_instance_attributes.lean")],
        &KVMap::new(),
        limits(),
...
fn late_unknown_or_nonclass_names_publish_no_attribute_prefix() {
    ...
    for suffix in ["missing", "plain"] {
        ...
        assert!(base.check_source_files(&[source.as_bytes()], &KVMap::new(), limits()).is_err());
        assert_eq!(before, base.logical_root(&KVMap::new()));
        assert!(!InstanceRegistry::read(base.environment()).unwrap()
            .candidates(&n("Inhabited")).iter().any(|entry| entry.declaration == n("seven")));
...
fn global_instance_attributes_survive_scope_and_source_file_boundaries() {
    ...
    let result = base.check_source_files(&[one.as_slice(), two.as_slice()], &KVMap::new(), limits())
        .unwrap().into_complete().unwrap();
    assert_eq!(result.files, 2);
    assert_eq!(result.theorems, 2);
    assert_eq!(InstanceRegistry::read(result.engine.environment()).unwrap()
        .candidates(&n("Inhabited"))[0].declaration, n("Exported.dictionary"));
}
...
fn oversized_attribute_lists_are_resource_stops_without_publication() {
    ...
    let error = base.check_source_files(&[source.as_bytes()], &KVMap::new(), limits()).unwrap_err();
...
fn updates_replay_across_imports_and_invalidate_cached_consumers() {
    ...
    const USE_SEVEN: &str = "prelude\nimport Adjusted\ntheorem importedSelection : default = 7 := by rfl";
    const USE_NINE: &str = "prelude\nimport Adjusted\ntheorem importedSelection : default = 9 := by rfl";
    fn inputs<'a>(names: &'a [Name; 3], update: &'a str, consumer: &'a str)
        -> [SourceModuleInput<'a>; 3]
    {
        [
            SourceModuleInput { name: &names[0], source: DICTIONARIES.as_bytes() },
            SourceModuleInput { name: &names[1], source: update.as_bytes() },
            SourceModuleInput { name: &names[2], source: consumer.as_bytes() },
        ]
    }
    let base = engine();
    let names = [n("Dictionaries"), n("Adjusted"), n("Consumer")];
    let module_limits = SourceModuleCheckLimits::new(limits());
    let mut session = SourceModuleSession::new(base.clone(), KVMap::new(), module_limits,
        SourceModuleCacheLimits::default());
    let first = session.check(&inputs(&names, HIGH, USE_SEVEN), &names[2])
        .unwrap().into_complete().unwrap();
    ...
    let warm = session.check(&inputs(&names, HIGH, USE_SEVEN), &names[2])
        .unwrap().into_complete().unwrap();
    ...
    assert!(session.check(&inputs(&names, LOW, USE_SEVEN), &names[2]).is_err());
    assert_eq!(session.retained_modules(), 3);
    let recovered = session.check(&inputs(&names, HIGH, USE_SEVEN), &names[2])
        .unwrap().into_complete().unwrap();
    ...
    let changed = session.check(&fixed, &names[2]).unwrap().into_complete().unwrap();
    ...
    let cold = base.check_source_modules(&fixed, &names[2], &KVMap::new(), module_limits)
        .unwrap().into_complete().unwrap();
    assert_eq!(changed.checked.checked.result_logical_root, cold.checked.result_logical_root);
}
