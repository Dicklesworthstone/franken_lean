...
fn type_positions_insert_class_receivers_in_result_arrow_let_and_ascription() {
    ...
    for (declaration, result) in [
        (
            "def answer : Factory.carrier := Factory.produce",
            "answer = (7 : Nat)",
        ),
        (
            "def answer : Factory.carrier -> Nat := fun x => x",
            "answer (7 : Nat) = 7",
        ),
        (
            "def answer : Nat -> Factory.carrier := fun x => x",
            "answer 7 = (7 : Nat)",
        ),
        (
            "def answer : Nat := let x : Factory.carrier := Factory.produce; x",
            "answer = 7",
        ),
        (
            "def answer : Nat := (Factory.produce : Factory.carrier)",
            "answer = 7",
        ),
        (
            "structure Wrapped where\n  value : Factory.carrier\ndef answer : Wrapped := { value := (7 : Nat) }",
            "answer.value = (7 : Nat)",
        ),
    ] {
        check(
...
fn type_positions_keep_anonymous_named_and_ordinary_local_dictionaries() {
    check(
        ...
        "def make [Factory] : Factory.carrier := Factory.produce\n\
         ...
         def localValue (f : Factory) : f.carrier := (Factory.produce : Factory.carrier)\n\
         theorem global : make = (7 : Nat) := by rfl\n\
         theorem kept : keep (9 : Nat) = (9 : Nat) := by rfl\n\
         theorem selected : localValue ({ carrier := Nat, produce := 9 }) = (9 : Nat) := by rfl",
    );
}
...
fn type_positions_allow_later_header_constraints_before_resolving_instances() {
    check(
        ...
        "def inferred {A : Type} [Inhabited A] : Type := A\n\
         ...
         theorem result : keep (0 : Nat) (by rfl) = 0 := by rfl",
    );
}
...
fn type_positions_preserve_supported_let_scope_and_refuse_nested_let_syntax() {
    ...
    check(
        &base,
        "def carrier : Type := let d : Factory := { carrier := Nat, produce := 7 }; Factory.carrier\n\
         def keep (x : carrier) : Nat := x\n\
         theorem result : keep (8 : Nat) = 8 := by rfl",
    );
}
...
fn type_positions_preserve_pending_outer_and_nested_annotation_inference() {
    check(
        ...
        "def choose {A : Type} [Inhabited A] (x : A) : A := default\n\
         ...
         def localValue (n : Nat) : Nat := let x : inferred := n; x\n\
         def ordinary (x : family) := (x : Nat)\n\
         theorem ascribed_ok : ascribed = 0 := by rfl\n\
         theorem nested_ok : nested = 0 := by rfl\n\
         theorem identity_ok : identity 7 = 7 := by rfl\n\
         theorem local_ok : localValue 9 = 9 := by rfl\n\
         theorem ordinary_ok : ordinary (8 : Nat) = (8 : Nat) := by rfl",
    );
}
...
#[test]
fn type_positions_preserve_named_class_inputs_through_ascriptions() {
    let base = check(
        &engine(),
        "class Choice (A : Type) where\n  value : A\n\
         def Alias : Type := Nat\n\
         instance natChoice : Choice Nat := { value := 0 }\n\
         instance aliasChoice : Choice Alias := { value := (9 : Nat) }\n\
         def choose {A : Type} [Choice A] (x : A) : A := Choice.value",
    )
    .engine;
    for (domain, expected) in [("Alias", 9), ("Nat", 0)] {
        check(
            &base,
            &format!(
                "def selected (n : {domain}) := choose (n : Alias)\n\
                 theorem result : selected (1 : Nat) = ({expected} : Nat) := by rfl"
            ),
        );
    }
}
