...
fn leaf_edits_reuse_dependencies_and_a_sibling_edit_invalidates_only_its_consumers() {
    ...
    assert_eq!(
        original.checked.checked.engine.logical_root(&KVMap::new()),
        original_root
    );
...
fn cache_cannot_lend_removed_imports_names_or_metadata() {
    ...
    assert!(
        check_with(
            &mut session,
            &[("Main", "theorem stolen (n : Nat) : wrap n = n := by simp")],
            None
        )
        .is_err()
    );
    ...
    assert!(
        fln_elab::source::scope::simp::read(repaired.checked.checked.engine.environment())
            .unwrap()
            .is_empty()
    );
}
...
fn cancellation_also_vetoes_fully_cached_publication_and_keeps_the_prior_closure() {
    ...
    for after in [0, 2, 5] {
        ...
        assert!(matches!(
            check_with(&mut session, &GRAPH, Some(&stop)),
...
fn retention_is_bounded_clearable_and_never_required_for_checking() {
    ...
    let mut disabled = SourceModuleSession::new(
        engine(),
        KVMap::new(),
        limits(),
        SourceModuleCacheLimits {
