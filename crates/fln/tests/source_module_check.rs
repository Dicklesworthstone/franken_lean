...
use fln::source_check::modules::{
    SourceModuleCheck, SourceModuleCheckError, SourceModuleCheckLimits,
};
use fln::{
    Budget, CancellationProbe, Engine, EngineAdmissionLimits, KVMap, Name, Outcome,
...
fn imported_polymorphic_theorems_use_the_ordinary_source_and_checker_path() {
    ...
    assert_eq!(
        ...
        result.checked.engine.logical_root(&KVMap::new())
    );
}
...
fn diamonds_replay_shared_declarations_and_simp_journals_once() {
    ...
    assert_eq!(
        result.module_order,
        [n("Base"), n("Left"), n("Right"), n("Main")]
    );
    assert_eq!(
        fln_elab::source::scope::simp::read(result.checked.engine.environment())
            .unwrap()
            .len(),
        1
    );
...
fn siblings_cannot_lend_names_or_simp_rules_to_an_unimporting_module() {
    ...
    assert!(
        matches!(result, Err(SourceModuleCheckError::Source { module, .. }) if module == n("B"))
    );
    let result = check(
        &base,
        &[
    ...
    checked(
        &base,
        &[("Main", "theorem recovery : (0 : Nat) = 0 := by rfl")],
    );
}
...
fn import_order_controls_instance_recency_not_input_array_order() {
    ...
    for (imports, expected) in [("A B", 9), ("B A", 7)] {
        ...
        let files = [
            ...
            ("B", "instance nine : Inhabited Nat := Inhabited.mk 9"),
        ];
        ...
        assert_eq!(
            first.checked.result_logical_root,
            second.checked.result_logical_root
        );
    }
}
...
fn missing_cyclic_duplicate_and_unreachable_modules_refuse_before_publication() {
    let base = engine();
    assert!(matches!(
        check(&base, &[("Main", "import Missing")]),
        Err(SourceModuleCheckError::MissingModule { .. })
    ));
    assert!(matches!(
        check(&base, &[("Main", "import A"), ("A", "import Main")]),
        Err(SourceModuleCheckError::Cycle(_))
    ));
    assert!(matches!(
        check(&base, &[("Main", ""), ("Main", "")]),
        Err(SourceModuleCheckError::DuplicateModule(_))
    ));
    assert!(matches!(
        check(&base, &[("Main", ""), ("Other", "def hidden := 1")]),
        Err(SourceModuleCheckError::UnreachableModule(_))
    ));
}
...
fn errors_retain_original_offsets_after_unicode_crlf_headers() {
    ...
    let result = check(
        &engine(),
        &[("Main", source), ("Base", "def imported := 1")],
    );
    let Err(SourceModuleCheckError::Source { module, error }) = result else {
        panic!("source error")
    };
    assert_eq!(module, n("Main"));
    let fln::SourceCheckError::Command { offset, .. } = error else {
        panic!("command error")
    };
...
fn resource_and_cancellation_stops_publish_no_successor() {
    ...
    let modules = [SourceModuleInput {
        name: &name,
        source: b"def ignored := 1",
    }];
    assert!(matches!(
        base.check_source_modules_with_cancel(
            &modules,
            &name,
            &KVMap::new(),
            limits(),
...
fn admission_only_graphs_never_run_evaluations_or_accept_false_imported_proofs() {
    ...
    for body in ["#eval 7", "theorem falseProof : (0 : Nat) = 1 := by rfl"] {
        assert!(!matches!(
            check(&base, &[("Main", "import A"), ("A", body)]),
...
fn extension_budgets_and_mid_replay_cancellation_retain_the_original_engine() {
    ...
    assert_eq!(
        check_with(&base, &files, low).unwrap_err().disposition(),
        ("resource", false, 3)
    );
