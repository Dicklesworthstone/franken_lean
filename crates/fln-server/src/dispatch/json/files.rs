...
pub(in crate::dispatch) fn watched_file_uris(
    params: RawField<'_>,
...
fn decode_events(
    ...
    if params.len() > max_bytes {
        return Err("watched-file changes exceed their byte limit");
    }
    ...
    while index < end && bytes.get(index) != Some(&b']') {
        count += 1;
        if count > max_events {
            return Err("watched-file changes exceed their event limit");
        }
        ...
        let uri = match decoded_string_field(event, "uri") {
            DecodedField::Valid(uri) if !uri.is_empty() && !uri.chars().any(char::is_control) => {
                uri
            }
            _ => return Err("watched-file events require one nonempty URI string"),
        };
        if !matches!(
            decoded_integer_field(event, "type"),
            VersionField::Valid(1..=3)
        ) {
...
pub(in crate::dispatch) enum RegistrationReply {
    Accepted,
    Rejected,
    Malformed,
}
...
pub(in crate::dispatch) fn registration_reply(message: &str) -> RegistrationReply {
    if object_field(message, "params") != RawField::Missing {
        return RegistrationReply::Malformed;
    }
    match (
        object_field(message, "result"),
        object_field(message, "error"),
    ) {
        ...
        (RawField::Missing, RawField::Value(error))
            if matches!(decoded_integer_field(RawField::Value(error), "code"),
                ...
                && matches!(
                    decoded_string_field(RawField::Value(error), "message"),
                    DecodedField::Valid(_)
                ) =>
        {
            RegistrationReply::Rejected
        }
...
mod tests {
    use super::*;
    fn decode(params: &str) -> Result<Vec<String>, &'static str> {
        watched_file_uris(RawField::Value(params))
    }
    #[test]
    ...
    fn a_malformed_late_event_never_returns_the_valid_prefix() {
        for bad in [
            ...
            r#"{"uri":"file:///b","type":4}"#,
            r#"{"uri":"file:///b","type":"2"}"#,
            r#"{"uri":"file:///b","type":2.0}"#,
            r#"{"uri":"file:///b","type":2,"type":2}"#,
            r#"{"uri":"a","uri":"b","type":2}"#,
            r#"{"uri":"\ud800","type":2}"#,
        ] {
    ...
    fn repeated_events_still_consume_count_and_byte_budgets() {
        ...
        assert_eq!(
            decode_events(RawField::Value(params), 2, params.len()).unwrap(),
            ["file:///a"]
        );
    }
    #[test]
    fn capability_is_exact_and_nested_decoys_or_duplicates_cannot_enable_it() {
        ...
        for no in [
            ...
            r#"{"capabilities":{"workspace":{"didChangeWatchedFiles":{"other":{"dynamicRegistration":true}}}}}"#,
            r#"{"capabilities":{},"capabilities":{"workspace":{"didChangeWatchedFiles":{"dynamicRegistration":true}}}}"#,
        ] {
            assert!(!supports_file_registration(RawField::Value(no)), "{no}");
        }
        for value in ["false", "null", "1", "\"true\""] {
            assert!(!supports_file_registration(RawField::Value(
                &yes.replace("true", value)
            )));
        }
    }
    #[test]
    fn registration_responses_require_an_exclusive_null_result_or_typed_error() {
        assert_eq!(
            registration_reply(r#"{"result":null}"#),
            RegistrationReply::Accepted
        );
        assert_eq!(
            registration_reply(r#"{"error":{"code":-32601,"message":"unsupported"}}"#),
            RegistrationReply::Rejected
        );
        for bad in [
            "{}",
            r#"{"result":{}}"#,
            r#"{"result":null,"result":null}"#,
            r#"{"result":null,"params":{}}"#,
            r#"{"result":null,"error":{"code":1,"message":"bad"}}"#,
            r#"{"error":{"code":2147483648,"message":"bad"}}"#,
            r#"{"error":{"code":1,"message":false}}"#,
        ] {
            assert_eq!(
                registration_reply(bad),
                RegistrationReply::Malformed,
                "{bad}"
            );
        }
    }
