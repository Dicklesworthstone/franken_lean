...
impl Registration {
    ...
    pub(super) fn start(&mut self, output: &mut dyn Write) -> io::Result<()> {
        if !self.supported || self.requested { return Ok(()); }
        self.requested = true;
        self.pending = true;
        write_protocol_message(output, format!(
    ...
    pub(super) fn stop(&mut self) { self.pending = false; }
    ...
    pub(super) fn consume(&mut self, output: &mut dyn Write, message: &str, envelope: &Envelope<'_>) -> io::Result<bool> {
        if envelope.method != DecodedField::Missing
            || !matches!(&envelope.id, RequestIdField::Valid(RequestId::Text(id)) if id == REQUEST_ID)
        { return Ok(false); }
        if !self.pending {
            write_warning(output, "FrankenLean ignored an unsolicited or duplicate file-watcher registration response")?;
            return Ok(true);
        }
        self.pending = false;
        match registration_reply(message) {
            RegistrationReply::Accepted => {}
            RegistrationReply::Rejected => write_warning(output, "Client declined Lean source file watching; editor buffer checking remains available")?,
            RegistrationReply::Malformed => write_warning(output, "Client sent a malformed file-watcher registration response; watching is not confirmed")?,
        }
        Ok(true)
    }
}
...
mod tests {
    use super::*;
    const CAPS: &str = r#"{"capabilities":{"workspace":{"didChangeWatchedFiles":{"dynamicRegistration":true}}}}"#;
    fn reply(value: &str) -> String { format!(r#"{{"jsonrpc":"2.0","id":"{REQUEST_ID}",{value}}}"#) }
    #[test]
    fn registration_is_capability_gated_and_sent_once() {
        ...
        state.start(&mut out).unwrap();
        assert_eq!(once, out);
        let body = transport::read_message(&mut std::io::Cursor::new(out)).unwrap().unwrap();
        let body = String::from_utf8(body).unwrap();
        assert_eq!(parse_envelope(&body).unwrap().method, DecodedField::Valid("client/registerCapability".to_owned()));
    ...
    fn response_ids_roles_and_completion_are_not_document_authority() {
        ...
        let mut out = Vec::new();
        for body in [reply(r#""method":"textDocument/hover","params":{}"#), r#"{"jsonrpc":"2.0","id":"other","result":null}"#.to_owned()] {
            assert!(!state.consume(&mut out, &body, &parse_envelope(&body).unwrap()).unwrap());
    ...
    fn rejected_malformed_and_shutdown_replies_do_not_retry_or_break_the_session() {
        for value in [r#""error":{"code":-32601,"message":"no"}"#, r#""result":{}"#, r#""result":null,"error":{}"#] {
            let mut state = Registration::default();
            state.configure(RawField::Value(CAPS), true);
            state.start(&mut Vec::new()).unwrap();
