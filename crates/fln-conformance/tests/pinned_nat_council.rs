...
fn preflight_extended_18_modules_council_fast_admission() {
    ...
    assert_eq!(total_new_decls, 1840);
    eprintln!(
        "ALL 18 EXTENDED MODULES ({total_new_decls} DECLARATIONS) VERIFIED THROUGH TWO-CHECKER COUNCIL!"
    );
}
...
fn preflight_extended_3_modules_to_63_council_fast_admission() {
    ...
    eprintln!(
        "Preloaded 60-module base environment has {} constants",
        env.len()
    );
...
fn scan_downstream_candidates() {
    ...
    eprintln!("\n=== IMMEDIATE CANDIDATE MODULES (ALL IMPORTS SATISFIED BY CURRENT 38) ===");
...
fn preflight_candidate_56_modules() {
    ...
    eprintln!("\nCandidate declaration counts:");
    for (name, count) in candidate_decl_counts {
        eprintln!("  {name}: {count}");
    }
    assert_eq!(total_decls, 7822);
    eprintln!(
        "ALL 18 CANDIDATE MODULES HAVE STRICTLY SATISFIED IMPORTS AND ZERO MISSING CONSTANTS! (Total declarations: {total_decls})"
    );
}
...
fn preflight_candidate_59_modules() {
    ...
    assert_eq!(total_decls, 8396);
    eprintln!(
        "ALL CANDIDATE MODULES HAVE STRICTLY SATISFIED IMPORTS AND ZERO MISSING CONSTANTS! (Total declarations: {total_decls})"
    );
}
...
fn preflight_candidate_63_modules() {
    ...
    let candidate_modules = ["Init/ByCases", "Init/Data/Function", "Init/MethodSpecsSimp"];
    ...
    for name in &candidate_modules {
        ...
        for imp in &m.module.imports {
            ...
            assert!(
                set_of_all.contains(&imp.module),
                "Candidate {name} imports {} which is not available!",
                imp.module.to_display_string()
            );
        }
        for c in &m.constants {
            if let Some(prev_mod) = available_consts.get(c.name()) {
                if c.name().to_display_string() == "ite.congr_simp" {
                    eprintln!(
                        "BENIGN KNOWN REPEAT in {name}: {} (previously in {prev_mod})",
                        c.name().to_display_string()
                    );
                } else {
                    eprintln!(
                        "DUPLICATE CONSTANT in {name}: {} (previously in {prev_mod})",
                        c.name().to_display_string()
                    );
                    duplicates.push((*name, c.name().to_display_string(), *prev_mod));
                }
    ...
    assert!(
        duplicates.is_empty(),
        "No unexpected duplicate constants allowed: {duplicates:?}"
    );
    ...
    assert_eq!(total_decls, 8546);
    eprintln!(
        "ALL 63 CANDIDATE MODULES HAVE STRICTLY SATISFIED IMPORTS AND ZERO MISSING CONSTANTS! (Total declarations: {total_decls})"
    );
}
...
fn diagnose_candidate_modules() {
    ...
    eprintln!("\n=== IMMEDIATE CANDIDATE MODULES (ALL IMPORTS SATISFIED BY CURRENT 63) ===");
    for (rel_path, name, count) in &immediate_candidates {
        eprintln!(
            "  Candidate: {} ({}) with {} declarations",
            rel_path, name, count
        );
    }
}
...
fn pinned_init_control_companion_chain_council_run() {
    ...
    match result {
        Ok(Outcome::Complete(checked)) => {
            ...
            assert_eq!(checked.modules[5].declarations.len(), 174);
            assert_eq!(checked.modules[6].declarations.len(), 1152);
            assert_eq!(checked.modules[7].declarations.len(), 108);
            assert_eq!(checked.modules[8].declarations.len(), 30);
...
fn pinned_extended_25_module_companion_chain_council_run() {
    ...
    match result {
        Ok(Outcome::Complete(checked)) => {
            ...
            assert_eq!(checked.modules[5].declarations.len(), 174);
            assert_eq!(checked.modules[6].declarations.len(), 1152);
            assert_eq!(checked.modules[7].declarations.len(), 28);
            assert_eq!(checked.modules[8].declarations.len(), 43);
            assert_eq!(checked.modules[9].declarations.len(), 108);
            assert_eq!(checked.modules[10].declarations.len(), 30);
            assert_eq!(checked.modules[11].declarations.len(), 12);
            assert_eq!(checked.modules[12].declarations.len(), 62);
            assert_eq!(checked.modules[13].declarations.len(), 9);
            assert_eq!(checked.modules[14].declarations.len(), 33);
            assert_eq!(checked.modules[15].declarations.len(), 15);
            assert_eq!(checked.modules[16].declarations.len(), 9);
            assert_eq!(checked.modules[17].declarations.len(), 7);
            assert_eq!(checked.modules[18].declarations.len(), 7);
            assert_eq!(checked.modules[19].declarations.len(), 13);
            assert_eq!(checked.modules[20].declarations.len(), 4);
            assert_eq!(checked.modules[21].declarations.len(), 1);
            assert_eq!(checked.modules[22].declarations.len(), 109);
            assert_eq!(checked.modules[23].declarations.len(), 24);
            assert_eq!(checked.modules[24].declarations.len(), 16);
        }
        Ok(Outcome::Inconclusive(reason)) => {
            panic!("INCONCLUSIVE: {reason:?}");
        }
...
fn pinned_extended_38_module_companion_chain_council_run() {
    ...
    match result {
        Ok(Outcome::Complete(checked)) => {
            ...
            assert_eq!(checked.modules[6].declarations.len(), 1152);
            assert_eq!(checked.modules[7].declarations.len(), 28);
            assert_eq!(checked.modules[8].declarations.len(), 199);
            assert_eq!(checked.modules[9].declarations.len(), 43);
            assert_eq!(checked.modules[10].declarations.len(), 108);
            assert_eq!(checked.modules[11].declarations.len(), 30);
            assert_eq!(checked.modules[12].declarations.len(), 12);
            assert_eq!(checked.modules[13].declarations.len(), 62);
            assert_eq!(checked.modules[14].declarations.len(), 9);
            assert_eq!(checked.modules[15].declarations.len(), 33);
            assert_eq!(checked.modules[16].declarations.len(), 15);
            assert_eq!(checked.modules[17].declarations.len(), 9);
            assert_eq!(checked.modules[18].declarations.len(), 7);
            assert_eq!(checked.modules[19].declarations.len(), 7);
            assert_eq!(checked.modules[20].declarations.len(), 13);
            assert_eq!(checked.modules[21].declarations.len(), 24);
            assert_eq!(checked.modules[22].declarations.len(), 1);
            assert_eq!(checked.modules[23].declarations.len(), 28);
            assert_eq!(checked.modules[24].declarations.len(), 100);
            assert_eq!(checked.modules[25].declarations.len(), 4);
            assert_eq!(checked.modules[26].declarations.len(), 8);
            assert_eq!(checked.modules[27].declarations.len(), 4);
            assert_eq!(checked.modules[28].declarations.len(), 3);
            assert_eq!(checked.modules[29].declarations.len(), 477);
            assert_eq!(checked.modules[30].declarations.len(), 120);
            assert_eq!(checked.modules[31].declarations.len(), 1);
            assert_eq!(checked.modules[32].declarations.len(), 109);
            assert_eq!(checked.modules[33].declarations.len(), 24);
            assert_eq!(checked.modules[34].declarations.len(), 1);
            assert_eq!(checked.modules[35].declarations.len(), 18);
            assert_eq!(checked.modules[36].declarations.len(), 25);
            assert_eq!(checked.modules[37].declarations.len(), 16);
        }
        Ok(Outcome::Inconclusive(reason)) => {
            panic!("INCONCLUSIVE: {reason:?}");
        }
    ...
    match result {
        Ok(Outcome::Complete(checked)) => {
            ...
            let checked_map: std::collections::BTreeMap<fln_core::name::Name, usize> = checked
                .modules
                .iter()
                .map(|m| (m.name.clone(), m.declarations.len()))
                .collect();
            for (expected_path, expected_count) in &module_specs {
...
fn pinned_extended_63_module_companion_chain_council_run() {
    ...
    match result {
        Ok(Outcome::Complete(checked)) => {
            ...
            assert_eq!(checked.modules.len(), 63);
            let mut total_verified = 0;
            let checked_map: std::collections::BTreeMap<fln_core::name::Name, usize> = checked
                .modules
                .iter()
            ...
            assert_eq!(total_verified, 8546);
            eprintln!(
                "ALL 63 MODULES ({total_verified} DECLARATIONS) FULLY VERIFIED IN PARALLEL COUNCIL!"
            );
        }
        Ok(Outcome::Inconclusive(reason)) => {
            panic!("INCONCLUSIVE: {reason:?}");
