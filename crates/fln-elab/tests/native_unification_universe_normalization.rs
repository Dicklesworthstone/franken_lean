...
fn txn() -> ElabTxn {
    ElabTxn::new(Environment::new(), KVMap::new(), 71)
}
...
fn imax_uses_definite_positivity_but_does_not_guess_a_guard() {
    ...
    tx.unify_many_with(
        &[
            ...
            pair(
                imax(p("u"), max(p("v"), s(p("w")))),
                max(max(p("w").succ().unwrap(), p("u")), p("v")),
            ),
...
fn final_publication_cancellation_rolls_back_normalized_universes() {
    ...
    assert!(matches!(
        cancelled.unify_many_with(&equations, generous, &|| {
