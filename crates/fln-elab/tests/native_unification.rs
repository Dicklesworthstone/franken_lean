...
fn pi(domain: Expr, body: Expr) -> Expr { Expr::forall_e(name("x"), domain, body, BinderInfo::Default) }
fn identity() -> Expr { Expr::lam(name("x"), nat(), bvar(0), BinderInfo::Default) }
...
fn ground_assignment_is_checked_without_publishing_a_declaration() {
    ...
    let report = txn.unify(&Expr::mvar(id.clone()), &numeral(37), budget()).unwrap();
...
fn wrong_typed_assignment_is_vetoed_by_the_real_kernel() {
    ...
    let error = txn.unify(&Expr::mvar(id.clone()), &Expr::sort(Level::zero()), budget()).unwrap_err();
    match error {
...
fn a_distinct_local_pattern_synthesizes_a_lambda() {
    ...
    let report = txn.unify(&Expr::app(Expr::mvar(f.clone()), x.clone()), &x, budget()).unwrap();
    assert_eq!(report.kernel_checks, 1);
    let assignment = txn.mvars.get_assigned_expr(&f).unwrap();
    let ExprNode::Lam { body, binder_type, .. } = assignment.node() else { panic!("expected lambda"); };
...
fn dependent_pattern_rebinds_later_domains_capture_avoidantly() {
    let mut txn = transaction();
    let f = natural(&mut txn, "dependent", pi(Expr::sort(Level::one()), pi(bvar(0), bvar(1))));
    ...
    assert_eq!(report.kernel_checks, 1);
    let assignment = txn.mvars.get_assigned_expr(&f).unwrap();
    let ExprNode::Lam { body, .. } = assignment.node() else { panic!("expected outer lambda"); };
    let ExprNode::Lam { binder_type, body, .. } = body.node() else { panic!("expected inner lambda"); };
...
fn lambda_comparison_opens_both_bodies_with_the_same_fresh_local() {
    let mut txn = transaction();
    let f = natural(&mut txn, "f", pi(nat(), nat()));
    let lhs = Expr::lam(name("left"), nat(), Expr::app(Expr::mvar(f.clone()), bvar(0)), BinderInfo::Default);
...
fn unresolved_assignment_typing_does_not_become_success() {
    ...
    let before = txn.clone();
    assert!(matches!(txn.unify(&Expr::mvar(value_hole), &numeral(1), budget()),
...
fn a_newer_local_cannot_escape_into_an_older_goal() {
    ...
    let before = txn.clone();
    assert!(matches!(txn.unify(&Expr::mvar(id), &x, budget()),
...
fn a_genuine_occurs_cycle_is_refused_without_mutation() {
    let mut txn = transaction();
    let id = natural(&mut txn, "n", nat());
    let rhs = Expr::app(Expr::const_(Name::from_components(["Nat", "succ"]), Vec::new()), Expr::mvar(id.clone()));
    let before = txn.clone();
    assert!(matches!(txn.unify(&Expr::mvar(id), &rhs, budget()),
...
fn opaque_and_deeper_metavariables_are_not_assigned() {
    for (kind, depth) in [(MetavarKind::SyntheticOpaque, 0), (MetavarKind::Natural, 1)] {
        ...
        let before = txn.clone();
        assert!(matches!(txn.unify(&Expr::mvar(id), &numeral(1), budget()), Err(UnificationError::Deferred(_))));
        assert_semantics_unchanged(&txn, &before);
    }
}
...
fn repeated_pattern_arguments_are_deferred() {
    ...
    let before = txn.clone();
    assert!(matches!(txn.unify(&lhs, &x, budget()), Err(UnificationError::Deferred(UnificationDeferred::NotAPattern))));
    assert_semantics_unchanged(&txn, &before);
}
...
fn universe_assignment_solves_a_sort_equation() {
    ...
    let report = txn.unify(&Expr::sort(Level::mvar(u.clone())), &Expr::sort(Level::one()), budget()).unwrap();
...
fn cyclic_universe_equations_defer_instead_of_constructing_a_cycle() {
    ...
    let u = LMVarId(name("u"));
    let before = txn.clone();
    assert!(matches!(txn.unify(&Expr::sort(Level::mvar(u.clone())),
        &Expr::sort(Level::mvar(u).succ().unwrap()), budget()),
...
fn rigid_failure_rolls_back_prior_assignments_and_constraint_extraction() {
    let mut txn = transaction();
    let id = natural(&mut txn, "n", nat());
    txn.postpone(ConstraintKind::HasType { expr: Expr::mvar(id.clone()), expected_type: nat() }, 0);
    let before = txn.clone();
    assert!(txn.unify_many_with(&[(Expr::mvar(id), numeral(7)), (numeral(1), numeral(2))], budget(), &|| false).is_err());
    assert_semantics_unchanged(&txn, &before);
}
...
fn successful_assignments_return_wakeups_in_stable_identity_order() {
    ...
    let first = txn.postpone(ConstraintKind::HasType { expr: Expr::mvar(b.clone()), expected_type: nat() }, 0);
    let second = txn.postpone(ConstraintKind::HasType { expr: Expr::mvar(a.clone()), expected_type: nat() }, 0);
    let report = txn.unify_many_with(&[(Expr::mvar(a), numeral(1)), (Expr::mvar(b), numeral(2))], budget(), &|| false).unwrap();
    assert_eq!(report.awakened.iter().map(|row| row.id).collect::<Vec<_>>(), vec![first, second]);
...
fn cancellation_at_the_final_publication_boundary_is_atomic() {
    ...
    control.unify_many_with(&equations, budget(), &|| { count.set(count.get() + 1); false }).unwrap();
    let limit = count.get();
    let count = Cell::new(0_usize);
...
fn zero_resource_budgets_do_not_publish_assignments() {
    for dimension in 0..3 {
        ...
        match dimension { 0 => limits.max_steps = 0, 1 => limits.max_visited_nodes = 0, _ => limits.max_assignments = 0 }
...
fn shared_reflexive_dags_are_not_expanded_as_trees() {
    ...
    let build = || {
        ...
        for _ in 0..60 { expr = Expr::app(expr.clone(), expr); }
        expr
    };
    let mut limits = budget();
...
fn loose_bound_variables_are_not_interpreted_as_locals() {
    let mut txn = transaction();
    assert!(matches!(txn.unify(&bvar(0), &bvar(0), budget()), Err(UnificationError::LooseBoundVariable)));
}
