...
fn local(txn: &mut ElabTxn, text: &str, type_: Expr) -> Expr {
    ...
    txn.lctx
        .add_param(id.clone(), name(text), type_, BinderInfo::Default);
    Expr::fvar(id)
}
...
fn a_later_function_type_revives_an_earlier_unavailable_assignment_hint() {
    for reverse_order in [false, true] {
        for reverse_sides in [false, true] {
            ...
            if reverse_sides {
                for (left, right) in &mut equations {
...
fn newly_inferred_types_unlock_further_hints_across_multiple_generations() {
    for order in [
        ...
        txn.unify(&Expr::mvar(t1), &function_type(), budget())
            .unwrap();
        txn.unify(&Expr::mvar(t2), &function_type(), budget())
            .unwrap();
    }
}
...
fn an_unavailable_hint_at_a_fixed_generation_defers_instead_of_spinning() {
    ...
    let error = txn
        .unify_many_with(&equations[..1], budget(), &|| false)
        .unwrap_err();
...
fn revisiting_typing_keeps_an_unrelated_postponed_equation_live() {
    ...
    equations.insert(
        0,
        (Expr::app(Expr::mvar(h.clone()), numeral(0)), numeral(1)),
    );
...
fn delayed_inference_exhaustion_restores_selected_and_unselected_queue_rows() {
    ...
    assert!(
        txn.solve_defeq_constraints_with(&ids, limited, &|| false)
            .is_err()
    );
    unchanged(&txn, &before);
    let report = txn
        .solve_defeq_constraints_with(&ids, budget(), &|| false)
        .unwrap();
...
fn repeated_runs_reconstruct_identical_assignments_without_publishing_declarations() {
    ...
    for _ in 0..3 {
        ...
        assert_eq!(
            ...
            first_report.expression_assignments
        );
