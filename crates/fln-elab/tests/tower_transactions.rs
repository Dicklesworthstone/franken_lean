...
fn test_five_transaction_outcomes_algebra() {
    ...
    child1.mvars.assign(mvar1.clone(), Expr::sort(Level::zero()), AssignmentJustification::DirectDefEq).unwrap();
    child1.messages.add(Message::info("child1 committed"));
    child1.lctx.add_param(FVarId(Name::from_components(["x"])), Name::from_components(["x"]), Expr::sort(Level::one()), BinderInfo::Default);

    root_txn.commit_outcome(&cp1, child1, TxnOutcome::CommitAll).unwrap();
    ...
    child2.mvars.assign(mvar2.clone(), Expr::sort(Level::zero()), AssignmentJustification::DirectDefEq).unwrap();
    child2.messages.add(Message::error("failed alternative in child2"));
    child2.info_tree.add_leaf(Info::CommandInfo { name: Name::from_components(["test_cmd"]) });
    child2.lctx.add_param(FVarId(Name::from_components(["y"])), Name::from_components(["y"]), Expr::sort(Level::one()), BinderInfo::Default);

    root_txn.commit_outcome(&cp2, child2, TxnOutcome::CommitDiagnosticsOnly).unwrap();
    ...
    child3.info_tree.add_leaf(Info::CommandInfo { name: Name::from_components(["leaked_cmd"]) });
    child3.lctx.add_param(FVarId(Name::from_components(["z"])), Name::from_components(["z"]), Expr::sort(Level::one()), BinderInfo::Default);

    root_txn.commit_outcome(&cp3, child3, TxnOutcome::Rollback).unwrap();
    // Verify zero state leaks
    assert_eq!(root_txn.mvars.len(), cp3.mvar_decls_count);
    assert_eq!(root_txn.mvars.assignments().len(), cp3.mvar_assignments_count);
    ...
    let candidate_res = root_txn.commit_outcome(
        &cp4,
        child4,
...
fn test_targeted_wake_up_precision_no_spurious_rescheduling() {
    ...
    store.declare(m1.clone(), Name::from_components(["m1"]), Expr::sort(Level::one()), LocalContext::new(), MetavarKind::Natural, 0, None);
    store.declare(m2.clone(), Name::from_components(["m2"]), Expr::sort(Level::one()), LocalContext::new(), MetavarKind::Natural, 0, None);
    store.declare(m3.clone(), Name::from_components(["m3"]), Expr::sort(Level::one()), LocalContext::new(), MetavarKind::Natural, 0, None);

    // C1 reads {m1}
    let mut reads_c1 = HashSet::new();
    reads_c1.insert(m1.clone());
    let id_c1 = queue.enqueue(ConstraintKind::DefEq { lhs: Expr::sort(Level::zero()), rhs: Expr::sort(Level::zero()) }, reads_c1, 0);

    // C2 reads {m2}
    let mut reads_c2 = HashSet::new();
    reads_c2.insert(m2.clone());
    let id_c2 = queue.enqueue(ConstraintKind::DefEq { lhs: Expr::sort(Level::zero()), rhs: Expr::sort(Level::zero()) }, reads_c2, 0);

    // C3 reads {m1, m3}
    let mut reads_c3 = HashSet::new();
    reads_c3.insert(m1.clone());
    reads_c3.insert(m3.clone());
    let id_c3 = queue.enqueue(ConstraintKind::DefEq { lhs: Expr::sort(Level::zero()), rhs: Expr::sort(Level::zero()) }, reads_c3, 0);
...
fn test_mvar_occurs_check_refusal() {
    ...
    let cyclic_val = Expr::app(Expr::const_(Name::from_components(["f"]), Vec::new()), Expr::mvar(m1.clone()));
    let err = store.assign(m1.clone(), cyclic_val, AssignmentJustification::DirectDefEq).unwrap_err();
...
fn test_synthetic_opaque_blocked_from_direct_defeq() {
    ...
    store.declare(m_opaque.clone(), Name::from_components(["m_opaque"]), Expr::sort(Level::one()), LocalContext::new(), MetavarKind::SyntheticOpaque, 0, None);

    let err = store.assign(m_opaque.clone(), Expr::sort(Level::zero()), AssignmentJustification::DirectDefEq).unwrap_err();
