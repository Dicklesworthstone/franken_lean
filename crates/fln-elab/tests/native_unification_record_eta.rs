...
fn dependent_function_results_infer_record_parameters_before_fields() {
    ...
    let f = local(
        ...
        Expr::forall_e(
            name("A"),
            Expr::sort(Level::one()),
            Expr::app(constant("Box"), Expr::bvar(0).unwrap()),
            BinderInfo::Default,
        ),
    );
...
fn empty_records_still_compare_and_infer_parameters() {
    ...
    let value = local(
        &mut txn,
        "p",
        Expr::app(constant("EmptyBox"), constant("Nat")),
    );
    let a = goal(&mut txn, "A", Expr::sort(Level::one()));
    let report = txn
    ...
    let before = txn.clone();
    assert!(matches!(
        txn.unify(&mk("EmptyBox", [constant("Bool")]), &value, budget()),
...
fn empty_records_do_not_erase_universe_constraints() {
    ...
    let value = local(
        &mut txn,
        "p",
        Expr::const_(name("UniverseUnit"), vec![Level::one()]),
    );
...
fn later_type_assignments_wake_blocked_record_equations() {
    ...
    let report = txn
        .unify_many_with(
            &[
                (record, value.clone()),
                (Expr::mvar(t.clone()), constant("PairN")),
            ],
            budget(),
            &|| false,
        )
...
fn malformed_constructor_arity_and_universes_fail_closed() {
    ...
    for constructor in [
        ...
        apply(
            Expr::const_(name("PairN.mk"), vec![Level::one()]),
            [numeral(0), numeral(1)],
        ),
    ] {
        let before = txn.clone();
        assert!(matches!(
            txn.unify(&constructor, &p, budget()),
            Err(UnificationError::Deferred(_))
        ));
        unchanged(&txn, &before);
    }
...
fn recursive_multiconstructor_and_indexed_families_do_not_take_record_eta() {
    ...
    let mut txn = transaction();
    let index = binder("index", constant("Nat"));
    // Use a Type-valued indexed family here: equality proofs are now handled
    // by proof irrelevance, which is distinct from structure eta.
    let block = inductive_declaration(
        &InductiveSpec {
            name: name("At"),
            level_params: vec![],
            parameters: vec![],
            indices: vec![binder("i", constant("Nat"))],
            constructors: vec![ConstructorSpec {
                name: name("mk"),
                fields: vec![index.clone(), binder("value", constant("Nat"))],
                result_indices: vec![Expr::fvar(index.id)],
            }],
            result_level: Level::one(),
        },
    ...
    let indexed = local(&mut txn, "indexed", apply(constant("At"), [numeral(0)]));
    for (constructor, value) in [
        (constant("Nat.zero"), n),
        (constant("Bool.true"), b),
        (apply(constant("At.mk"), [numeral(0), numeral(1)]), indexed),
    ] {
        let before = txn.clone();
        assert!(matches!(
            txn.unify(&constructor, &value, budget()),
            Err(UnificationError::Deferred(_))
        ));
        unchanged(&txn, &before);
    }
...
#[test]
fn indexed_equality_proofs_use_kernel_validation_not_record_eta() {
    let mut txn = transaction();
    let equality = apply(
    ...
    let before = txn.clone();
    let report = txn.unify(&refl, &h, budget()).unwrap();
    assert_eq!(
        report.kernel_checks, 1,
        "proof irrelevance requires K1 validation"
    );
    assert!(report.expression_assignments.is_empty());
    unchanged(&txn, &before);
}
...
fn record_eta_does_not_widen_local_let_transparency() {
    ...
    txn.lctx
        .add_let(id.clone(), id.0.clone(), constant("PairN"), record.clone());
    let p = Expr::fvar(id);
    let mut closed = budget();
    closed.zeta_delta = false;
    let before = txn.clone();
    assert!(matches!(
        txn.unify(&record, &p, closed),
        Err(UnificationError::Deferred(_))
    ));
    unchanged(&txn, &before);
...
fn add_holder(txn: &mut ElabTxn) {
    add_record(
        txn,
        spec("Holder", vec![], vec![binder("pair", constant("PairN"))]),
    );
}
...
fn dependent_record_fields_are_checked_after_earlier_field_inference() {
    ...
    let mut pack = spec(
        "Pack",
        vec![],
        vec![a.clone(), binder("value", Expr::fvar(a.id))],
    );
...
fn projected_functions_can_produce_records() {
    ...
    add_record(
        &mut txn,
        spec("Factory", vec![], vec![binder("make", function_type)]),
    );
...
fn a_later_assignment_can_reveal_a_projected_receivers_record_type() {
    ...
    let report = txn
        .unify_many_with(
            &[
                (record, value.clone()),
                (Expr::mvar(t.clone()), constant("Holder")),
            ],
            budget(),
            &|| false,
        )
...
fn nested_parameterized_projection_chains_are_metered() {
    ...
    for _ in 0..32 {
        type_ = Expr::app(constant("Box"), type_);
    }
    let mut value = local(&mut txn, "p", type_);
    for _ in 0..32 {
        value = proj("Box", 0, &value);
    }
    let record = mk(
        "PairN",
        [proj("PairN", 0, &value), proj("PairN", 1, &value)],
    );
    ...
    assert!(matches!(
        stopped.unify(&record, &value, limited),
        Err(UnificationError::StepLimit { .. })
    ));
...
fn record_eta_composes_with_function_eta_and_miller_patterns() {
    ...
    for (index, id) in [(0, x), (1, y)] {
        ...
        let report = txn
            .unify(&application, &proj("PairN", index, &value), budget())
            .unwrap();
        assert!(report.expression_assignments.is_empty());
    }
}
