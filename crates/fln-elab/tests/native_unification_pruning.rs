...
fn same_head_keeps_only_agreeing_positions_and_reports_the_typed_residual() {
    for reverse in [false, true] {
        ...
        txn.unify(&Expr::mvar(residual), &lam(nat(), bvar(0)), budget())
            .unwrap();
        txn.unify(
            &app(Expr::mvar(f), &[number(7), number(99)]),
            &number(7),
            budget(),
        )
        .unwrap();
...
fn middle_positions_survive_a_nontrivial_permutation() {
    ...
    txn.unify(&Expr::mvar(residual), &lam(nat(), bvar(0)), budget())
        .unwrap();
    txn.unify(
        &app(Expr::mvar(f), &[number(1), number(2), number(3)]),
        &number(2),
        budget(),
    )
    .unwrap();
...
fn dependent_results_keep_their_type_parameter_without_inventing_an_inhabitant() {
    ...
    let ExprNode::ForallE {
        binder_type, body, ..
    } = txn.mvars.get_decl(id).unwrap().type_.node()
    else {
...
fn ordinary_batch_assignments_take_precedence_over_residual_creation() {
    ...
    let report = txn
        .unify_many_with(
            &[(left, right), (Expr::mvar(f.clone()), value)],
            budget(),
            &|| false,
        )
        .unwrap();
...
fn repeated_and_nonlocal_arguments_do_not_enter_the_pattern_fragment() {
    for repeat in [false, true] {
        ...
        let x = local(&mut txn, "x", nat());
        let y = local(&mut txn, "y", nat());
        let left_args = if repeat {
            vec![x.clone(), x.clone()]
        } else {
            vec![x.clone(), number(0)]
        };
        let right_args = if repeat {
            vec![y.clone(), y]
        } else {
            vec![x, number(1)]
        };
...
fn queued_obligations_retain_residuals_instead_of_misreporting_complete_proofs() {
    ...
    let waiting = txn.postpone(
        ConstraintKind::HasType {
            expr: Expr::mvar(f),
            expected_type: pi(nat(), pi(nat(), nat())),
        },
        0,
    );
    let report = txn
        .solve_defeq_constraints_with(&[selected, selected], budget(), &|| false)
        .unwrap();
...
fn resource_and_cancellation_stops_publish_neither_assignment_nor_residual() {
    ...
    assert!(matches!(
        ...
        Err(UnificationError::AssignmentLimit { limit: 0 })
    ));
    unchanged(&txn, &base);
...
fn a_late_unsolved_equation_rolls_back_pruning_and_can_be_retried() {
    ...
    assert!(matches!(
        txn.unify_many_with(
            &[(left.clone(), right.clone()), (number(0), number(1))],
            budget(),
            &|| false,
        ),
        Err(UnificationError::Deferred(_))
...
fn a_retained_dependent_binder_keeps_its_domain_and_can_be_solved_later() {
    ...
    let f = natural(
        &mut txn,
        "f",
        pi(Expr::sort(Level::one()), pi(bvar(0), pi(nat(), bvar(2)))),
    );
    let a = local(&mut txn, "A", Expr::sort(Level::one()));
    let x = local(&mut txn, "x", a.clone());
    ...
    txn.unify(
        &app(Expr::mvar(f), &[nat(), number(5), number(99)]),
        &number(5),
        budget(),
    )
    .unwrap();
...
fn distinct_heads_share_one_typed_residual_and_check_both_assignments() {
    for reverse in [false, true] {
        ...
        txn.unify(&Expr::mvar(residual), &lam(nat(), bvar(0)), budget())
            .unwrap();
        txn.unify(
            &app(Expr::mvar(f), &[number(4), number(5)]),
            &number(5),
            budget(),
        )
        .unwrap();
        txn.unify(
            &app(Expr::mvar(g), &[number(5), number(6)]),
            &number(5),
            budget(),
        )
        .unwrap();
...
fn distinct_arities_and_permuted_intersections_rebind_each_function_separately() {
    for reverse in [false, true] {
        ...
        .unwrap();
        txn.unify(
            &app(Expr::mvar(g), &[number(12), number(10), number(14)]),
            &expected,
            budget(),
        )
        .unwrap();
...
fn disjoint_arguments_leave_a_shared_unsolved_value_not_a_guessed_constant() {
    ...
    assert!(!txn.mvars.is_assigned(&residual));
    txn.unify(&Expr::mvar(residual), &number(29), budget())
        .unwrap();
    txn.unify(&Expr::app(Expr::mvar(f), number(7)), &number(29), budget())
        .unwrap();
    txn.unify(&Expr::app(Expr::mvar(g), number(9)), &number(29), budget())
        .unwrap();
}
...
fn subsequent_equations_can_prune_the_shared_residual_again() {
    ...
    let report = txn
        .unify_many_with(
            &[
                (
                    app(Expr::mvar(f.clone()), &[x.clone(), y.clone()]),
                    app(Expr::mvar(g.clone()), &[y.clone(), z.clone()]),
                ),
                (
                    app(Expr::mvar(f.clone()), &[x.clone(), z]),
                    app(Expr::mvar(g.clone()), &[y, x]),
                ),
            ],
            budget(),
            &|| false,
    ...
    let residual = report.residual_metavariables[0].clone();
    assert_eq!(txn.mvars.get_decl(&residual).unwrap().type_, nat());
    txn.unify(&Expr::mvar(residual), &number(13), budget())
        .unwrap();
    txn.unify(
        &app(Expr::mvar(f), &[number(1), number(2)]),
        &number(13),
        budget(),
    )
    .unwrap();
    txn.unify(
        &app(Expr::mvar(g), &[number(3), number(4)]),
        &number(13),
        budget(),
    )
    .unwrap();
...
fn different_captured_contexts_are_not_silently_merged() {
    let mut txn = transaction();
    let f = natural(&mut txn, "f", curried(1));
    let private = local(&mut txn, "capture", nat());
    let g = natural(&mut txn, "g", curried(1));
    let x = local(&mut txn, "x", nat());
    let y = local(&mut txn, "y", nat());
    let report = txn
    ...
    let before = txn.clone();
    assert!(
        txn.unify(&Expr::mvar(residual.clone()), &private, budget())
            .is_err()
    );
    unchanged(&txn, &before);
    txn.unify(&Expr::mvar(residual), &number(17), budget())
        .unwrap();
    for head in [f, g] {
        txn.unify(
            &Expr::app(Expr::mvar(head), number(4)),
            &number(17),
            budget(),
        )
        .unwrap();
...
fn a_kernel_veto_on_the_second_assignment_cannot_publish_the_first_one() {
    ...
    let g = natural(
        &mut txn,
        "g",
        pi(Expr::const_(name("UnknownDomain"), Vec::new()), nat()),
    );
    let x = local(&mut txn, "x", nat());
    let y = local(&mut txn, "y", nat());
    let before = txn.clone();
...
fn shared_residual_depth_preserves_the_stricter_parent_scope() {
    for (left_depth, right_depth) in [(0, 1), (1, 0), (1, 2), (2, 1), (1, 1)] {
        let mut txn = transaction();
        let f = hole(&mut txn, "f", curried(2), MetavarKind::Natural, left_depth);
        let g = hole(&mut txn, "g", curried(2), MetavarKind::Natural, right_depth);
        let x = local(&mut txn, "x", nat());
        let y = local(&mut txn, "y", nat());
        let z = local(&mut txn, "z", nat());
        let mut limits = budget();
        limits.max_metavar_depth = left_depth.max(right_depth);
        let report = txn
            .unify(
        ...
        if depth > 0 {
            ...
            assert!(matches!(
                txn.unify(&Expr::mvar(residual.clone()), &lam(nat(), bvar(0)), limits),
                Err(UnificationError::Deferred(
                    UnificationDeferred::MetavariableDepth(_)
                ))
            ));
            unchanged(&txn, &before);
        }
        limits.max_metavar_depth = depth;
        txn.unify(&Expr::mvar(residual), &lam(nat(), bvar(0)), limits)
            .unwrap();
    }
}
...
fn distinct_heads_retain_a_dependent_result_parameter_in_the_shared_telescope() {
    ...
    let ExprNode::ForallE {
        binder_type, body, ..
    } = txn.mvars.get_decl(residual).unwrap().type_.node()
    else {
