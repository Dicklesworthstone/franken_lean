...
fn relation() -> Expr {
    lambda("a", constant("Nat"), lambda("b", constant("Nat"),
        eq_at(Level::one(), constant("Nat"), bv(1), bv(0))))
}
fn quotient_type() -> Expr {
    apply(Expr::const_(name("Quot"), vec![Level::one()]),
        [constant("Nat"), relation()])
}
fn representative(value: Expr) -> Expr {
    apply(Expr::const_(name("Quot.mk"), vec![Level::one()]),
...
fn identity_respects() -> Expr {
    lambda("a", constant("Nat"), lambda("b", constant("Nat"),
        lambda("h", eq_at(Level::one(), constant("Nat"), bv(1), bv(0)), bv(0))))
}
fn lift_with(major: Expr, result_type: Expr, branch: Expr, proof: Expr) -> Expr {
    apply(Expr::const_(name("Quot.lift"), vec![Level::one(), Level::one()]),
        [constant("Nat"), relation(), result_type, branch, proof, major])
}
fn lift(major: Expr) -> Expr {
    lift_with(major, constant("Nat"), identity(), identity_respects())
}
fn induction(major: Expr) -> Expr {
    let motive = lambda("q", quotient_type(),
        eq_at(Level::one(), quotient_type(), bv(0), bv(0)));
    let branch = lambda("a", constant("Nat"),
        refl_at(Level::one(), quotient_type(), representative(bv(0))));
    apply(Expr::const_(name("Quot.ind"), vec![Level::one()]),
...
fn k1_proves_computation(
    ...
    let candidate = Declaration::Defn(DefinitionVal {
        base: ConstantVal {
            name: id.clone(),
            level_params: vec![],
            type_: eq_at(level.clone(), type_.clone(), original.clone(), expected.clone()),
        },
    ...
    assert!(matches!(outcome, Outcome::Complete(Verdict::Accepted { .. })),
...
fn quotient_initialization_requires_eq_and_rejects_forged_types() {
    assert!(matches!(check(&Environment::new(), &quotient::quotient_declaration(), budget().kernel),
        Outcome::Complete(Verdict::Rejected { .. })));
    let env = publish_block(&Environment::new(), fln_elab::seed::eq_seed_declaration());
    let Declaration::Quotient(mut rows) = quotient::quotient_declaration() else { unreachable!() };
    rows[2].base.type_ = Expr::sort(Level::one());
    assert!(matches!(check(&env, &Declaration::Quotient(rows), budget().kernel),
        Outcome::Complete(Verdict::Rejected { .. })));
}
...
fn quotient_induction_computes_a_dependent_proof() {
    ...
    k1_proves_computation(&txn, Level::zero(),
        eq_at(Level::one(), quotient_type(), q.clone(), q.clone()),
        &induction(q.clone()), &expected);
    let report = txn.unify(&induction(q.clone()), &expected, budget()).unwrap();
    assert_eq!(report.kernel_checks, 0);
    ...
    let proof = goal(&mut txn, "proof", eq_at(Level::one(), quotient_type(), q.clone(), q.clone()));
    let report = txn.unify(&Expr::mvar(proof.clone()), &induction(q), budget()).unwrap();
...
fn a_quotient_valued_assignment_is_validated_by_k1() {
    ...
    let report = txn.unify(&Expr::mvar(id.clone()), &value, budget()).unwrap();
    assert_eq!(report.kernel_checks, 1);
...
fn a_later_universe_assignment_reawakens_quotient_computation() {
    ...
    let expr = apply(Expr::const_(name("Quot.lift"), vec![Level::mvar(u.clone()), Level::one()]),
        [constant("Nat"), relation(), constant("Nat"), identity(), identity_respects(),
...
fn quotient_computation_preserves_trailing_function_arguments() {
    ...
    let proof = lambda("a", constant("Nat"), lambda("b", constant("Nat"),
        lambda("h", eq_at(Level::one(), constant("Nat"), bv(1), bv(0)),
            refl_at(Level::one(), function_type.clone(), identity()))));
    let expr = Expr::app(lift_with(representative(numeral(5)), function_type, branch, proof), numeral(73));
...
fn lift_preserves_distinct_source_and_result_universes() {
    ...
    let r = lambda("A", alpha.clone(), lambda("B", alpha.clone(),
        eq_at(universe.clone(), alpha.clone(), bv(1), bv(0))));
    let q = apply(Expr::const_(name("Quot.mk"), vec![universe.clone()]),
        [alpha.clone(), r.clone(), constant("Nat")]);
    let f = lambda("A", alpha.clone(), numeral(9));
    let h = lambda("A", alpha.clone(), lambda("B", alpha.clone(),
        lambda("h", eq_at(universe.clone(), alpha.clone(), bv(1), bv(0)),
            refl_at(Level::one(), constant("Nat"), numeral(9)))));
    let expr = apply(Expr::const_(name("Quot.lift"), vec![universe, Level::one()]),
    ...
    let report = txn.unify(&expr, &Expr::mvar(out.clone()), budget()).unwrap();
    assert_eq!(report.kernel_checks, 1);
...
fn quotient_reduction_composes_with_beta_zeta_and_inductive_iota() {
    let mut txn = transaction();
    let choose = apply(Expr::const_(name("Bool.rec"), vec![Level::one()]), [
        lambda("b", constant("Bool"), quotient_type()),
        representative(numeral(4)), representative(numeral(9)), constant("Bool.true"),
    ]);
    let id = FVarId(name("chosen_quotient"));
    txn.lctx.add_let(id.clone(), id.0.clone(), quotient_type(), choose);
...
fn neutral_quotients_remain_blocked_and_unchanged() {
    ...
    let before = txn.clone();
    assert!(matches!(txn.unify(&lift(q), &numeral(37), budget()),
...
fn malformed_constructor_arities_and_universes_do_not_reduce() {
    for bad in [
        ...
        apply(Expr::const_(name("Quot.mk"), vec![Level::one()]), [constant("Nat"), relation()]),
        Expr::app(representative(numeral(37)), numeral(99)),
        apply(Expr::const_(name("Quot.mk"), vec![]), [constant("Nat"), relation(), numeral(37)]),
        apply(Expr::const_(name("Quot.mk"), vec![Level::zero()]), [constant("Nat"), relation(), numeral(37)]),
        apply(Expr::const_(name("User.Quot.mk"), vec![Level::one()]), [constant("Nat"), relation(), numeral(37)]),
    ] {
        let mut txn = transaction();
        let before = txn.clone();
        assert!(matches!(txn.unify(&lift(bad), &numeral(37), budget()),
            Err(UnificationError::Deferred(_))));
        unchanged(&txn, &before);
    }
}
...
fn partial_eliminators_and_wrong_universe_arity_do_not_reduce() {
    for expr in [
        apply(Expr::const_(name("Quot.lift"), vec![Level::one(), Level::one()]),
            [constant("Nat"), relation(), constant("Nat"), identity(), identity_respects()]),
        apply(Expr::const_(name("Quot.lift"), vec![Level::one()]),
            [constant("Nat"), relation(), constant("Nat"), identity(), identity_respects(),
                representative(numeral(37))]),
        apply(Expr::const_(name("Quot.ind"), vec![Level::one(), Level::one()]),
            [constant("Nat"), relation(), constant("Nat"), identity(), representative(numeral(37))]),
    ] {
        let mut txn = transaction();
        let before = txn.clone();
        assert!(matches!(txn.unify(&expr, &numeral(37), budget()),
            Err(UnificationError::Deferred(_))));
        unchanged(&txn, &before);
    }
}
...
fn quotient_step_cannot_publish_an_ill_typed_representative() {
    ...
    match txn.unify(&Expr::mvar(id.clone()), &expr, budget()).unwrap_err() {
...
fn quotient_computation_does_not_unfold_closed_local_lets() {
    ...
    txn.lctx.add_let(id.clone(), id.0.clone(), quotient_type(), representative(numeral(37)));
    let expr = lift(Expr::fvar(id));
    let before = txn.clone();
    let mut closed = budget();
    closed.zeta_delta = false;
    assert!(matches!(txn.unify(&expr, &numeral(37), closed),
        Err(UnificationError::Deferred(_))));
    unchanged(&txn, &before);
...
fn quotient_branches_feed_ordinary_higher_order_pattern_inference() {
    ...
    let compatibility = pi("a", constant("Nat"), pi("b", constant("Nat"),
        pi("h", eq_at(Level::one(), constant("Nat"), bv(1), bv(0)),
            eq_at(Level::one(), constant("Nat"),
                Expr::app(function.clone(), bv(2)), Expr::app(function.clone(), bv(1))))));
    let h = goal(&mut txn, "respectful", compatibility);
    let expr = lift_with(representative(x.clone()), constant("Nat"), function, Expr::mvar(h.clone()));
    let report = txn.unify(&expr, &x, budget()).unwrap();
    assert_eq!(report.expression_assignments, vec![f.clone()]);
    assert_eq!(report.kernel_checks, 1);
    assert!(txn.mvars.get_assigned_expr(&h).is_none(),
        "computation is not a proof of the respectfulness obligation");
    let value = txn.mvars.get_assigned_expr(&f).unwrap().clone();
    txn.unify(&Expr::app(value, numeral(13)), &numeral(13), budget()).unwrap();
}
...
fn quotient_driven_assignments_cannot_escape_their_local_scope() {
    ...
    let before = txn.clone();
    assert!(matches!(txn.unify(&Expr::mvar(id), &lift(representative(inner)), budget()),
        Err(UnificationError::Deferred(_))));
    unchanged(&txn, &before);
}
...
fn cancellation_problem(txn: &mut ElabTxn) -> Vec<(Expr, Expr)> {
    ...
    vec![(lift(representative(Expr::mvar(id))), numeral(37)), (value, numeral(37))]
}
...
fn final_barrier_cancellation_discards_quotient_assignments() {
    ...
    let report = control.unify_many_with(&equations, generous, &|| {
...
fn nested_quotient_majors_use_the_shared_heap_continuation_stack() {
    ...
    std::thread::Builder::new().stack_size(128 * 1024).spawn(move || {
        ...
        let compatibility = pi("a", constant("Nat"), pi("b", constant("Nat"),
            pi("h", eq_at(Level::one(), constant("Nat"), bv(1), bv(0)),
                eq_at(Level::one(), quotient_type(),
                    representative(bv(2)), representative(bv(1))))));
...
fn quotient_spelling_does_not_enable_uninitialized_primitives() {
    let env = publish_block(&Environment::new(), fln_elab::seed::nat_inductive_seed_declaration());
    let mut txn = ElabTxn::new(env, KVMap::new(), 41);
    let before = txn.clone();
    assert!(matches!(txn.unify(&lift(representative(numeral(37))), &numeral(37), budget()),
        Err(UnificationError::Deferred(_))));
    unchanged(&txn, &before);
}
