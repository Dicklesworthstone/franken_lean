//! Shared capture-preserving universe substitution. Replacements are simultaneous:
//! parameters appearing inside replacement levels are never substituted again.
//! The caller supplies its work counter and typed malformed-input error.
...
pub(crate) fn instantiate<E>(
    ...
    while let Some((current, exit)) = pending.pop() {
        ...
        let value = match current.node() {
            ExprNode::Sort { level } => Expr::sort(instantiate_param_level(
                &mut tick,
                &scope,
                level,
                &substitutions,
            ...
            ExprNode::Const { name, levels } => {
                ...
                for level in levels {
                    result.push(instantiate_param_level(
                        &mut tick,
                        &scope,
                        level,
                        &substitutions,
...
fn instantiate_param_level<E>(
    tick: &mut impl FnMut() -> Result<(), E>,
    scope: &impl Fn() -> E,
    level: &Level,
    ...
    while let Some((current, exit)) = pending.pop() {
        ...
        let result = match current.view() {
            ...
            LevelView::Succ(inner) => child(inner).succ().map_err(|_| scope())?,
            LevelView::Max(left, right) => {
                Level::max(child(left), child(right)).map_err(|_| scope())?
            }
            LevelView::IMax(left, right) => {
                Level::imax(child(left), child(right)).map_err(|_| scope())?
            }
            _ => current.clone(),
        };
