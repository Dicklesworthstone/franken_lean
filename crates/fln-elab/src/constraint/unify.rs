...
pub enum UnificationError {
    ...
    AssignmentCheck { id: MVarId, outcome: Box<Outcome<Verdict>> },
}
...
impl std::fmt::Display for UnificationError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::AssignmentLimit { limit } => write!(f, "unification assignment limit {limit} reached"),
            ...
            Self::ExpressionScope => write!(f, "unification substitution exceeded expression scope"),
            Self::Metavariable(error) => write!(f, "{error}"),
            Self::Universe(error) => write!(f, "{error}"),
            Self::AssignmentCheck { id, .. } => write!(f, "kernel did not validate assignment to ?{}", id.0.to_display_string()),
        }
    }
}
...
fn children(expr: &Expr) -> [Option<&Expr>; 3] {
    match expr.node() {
        ...
        | ExprNode::ForallE { binder_type, body, .. } => [Some(binder_type), Some(body), None],
...
fn facts(expr: &Expr, meter: &mut Meter<'_>) -> Result<Facts, UnificationError> {
    ...
    while let Some(current) = pending.pop() {
        ...
        match current.node() {
            ...
            ExprNode::Const { levels: arguments, .. } => levels.extend(arguments.iter().rev()),
            _ => {}
        }
    ...
    while let Some(level) = levels.pop() {
        ...
        match level.view() {
            ...
            LevelView::Max(a, b) | LevelView::IMax(a, b) => { levels.push(b); levels.push(a); }
            _ => {}
        }
    }
    Ok(result)
...
impl Engine<'_> {
                        ...
                        Some(ConstantInfo::Defn(definition))
                            if definition.safety == DefinitionSafety::Safe
                                ...
                                && match self.budget.transparency {
                                    ...
                                    UnificationTransparency::Abbreviations => definition.hints == ReducibilityHints::Abbrev,
    ...
    fn pattern(&mut self, lhs: &Expr, rhs: &Expr, locals: &LocalContext) -> Result<bool, UnificationError> {
        ...
        if declaration.kind == MetavarKind::SyntheticOpaque {
            return Err(UnificationError::Deferred(UnificationDeferred::OpaqueMetavariable(id.clone())));
        }
    ...
    fn levels(&mut self, left: &Level, right: &Level) -> Result<(), UnificationError> {
        ...
        while let Some((left, right)) = pending.pop() {
            self.meter.tick()?;
            let remaining = self.budget.max_visited_nodes.saturating_sub(self.meter.nodes);
            let left = self.work.universes.instantiate_with_limit(&left, remaining).map_err(UnificationError::Universe)?;
            ...
            match (left.view(), right.view()) {
                (LevelView::MVar(id), _) | (_, LevelView::MVar(id)) => {
                    ...
                    while let Some(level) = todo.pop() {
                        if !visited.insert(std::ptr::from_ref(level)) { continue; }
                        self.meter.node()?;
                        match level.view() {
                            LevelView::MVar(found) if found == id => return Err(UnificationError::Deferred(UnificationDeferred::CyclicUniverse(id.clone()))),
                            LevelView::Succ(inner) => todo.push(inner),
                            LevelView::Max(a, b) | LevelView::IMax(a, b) => { todo.push(b); todo.push(a); }
                            _ => {}
                        }
                    }
                    self.assignment_slot()?;
    ...
    fn solve(&mut self, equations: &[(Expr, Expr)]) -> Result<(), UnificationError> {
        ...
        for assignment in self.work.mvars.assignments().values() { roots.push(assignment.expr.clone()); }
    ...
    fn check_assignment(&mut self, id: &MVarId) -> Result<(), UnificationError> {
        ...
        for name in value_facts.params { if !params.contains(&name) { params.push(name); } }
        let mut ordinal = 0_u64;
        let name = loop {
            self.meter.tick()?;
            let suffix = ordinal.to_string();
            let name = Name::from_components(["_fln_unify_check", suffix.as_str()]);
            if !self.work.env.contains(&name) { break name; }
...
impl ElabTxn {
    pub fn unify(&mut self, lhs: &Expr, rhs: &Expr, budget: UnificationBudget) -> Result<UnificationReport, UnificationError> {
