...
impl Engine<'_> {
    fn validation_instantiate(
        ...
        let result = self.work.universes
    ...
    fn binding_reads(&mut self, expr: &Expr) -> Result<Vec<Binding>, UnificationError> {
        ...
        reads.extend(self.work.mvars.collect_mvars(&expanded).into_iter().map(Binding::Residual));
    ...
    fn prepare_binding(
        ...
        let (prepared, mut dependencies) = match &binding {
            Binding::Local(id) => {
                let position = allowed.decls().iter().position(|local| &local.id == id)
                    .ok_or_else(|| UnificationError::Deferred(UnificationDeferred::EscapingLocal(target.clone())))?;
            ...
            Binding::Residual(id) => {
                ...
                if declaration.depth > target_depth || declaration.depth > self.budget.max_metavar_depth {
                    return Err(UnificationError::Deferred(UnificationDeferred::MetavariableDepth(id.clone())));
                }
                let mut dependencies = Vec::new();
                ...
                for local in declaration.lctx.decls() {
                    ...
                    if !seen.insert(local.id.clone()) || allowed.find(&local.id) != Some(local) {
                        return Err(UnificationError::Deferred(UnificationDeferred::EscapingLocal(target.clone())));
                    }
    ...
    pub(super) fn prepare_assignment_check(
        ...
        let declaration = self.work.mvars.get_decl(target).cloned()
            .expect("the unifier assigns only declared metavariables");
        let raw_value = self.work.mvars.get_assigned_expr(target).cloned()
            ...
            match task {
                Task::Enter(binding) => {
                    ...
                    let (prepared, dependencies) = self.prepare_binding(
                        binding, target, &declaration.lctx, declaration.depth,
                    )?;
                    tasks.push(Task::Exit(prepared));
                    tasks.extend(dependencies.into_iter().rev().map(Task::Enter));
                }
                Task::Exit(prepared) => {
                    let domain = self.validation_instantiate(&prepared.domain, &validation_store)?;
                    let value = prepared.value.as_ref()
                    ...
                    for expr in std::iter::once(&domain).chain(value.iter()) {
                        ...
                        if self.scan(expr)?.fvars.iter().any(|id| !telescope.contains(id)) {
                            return Err(UnificationError::Deferred(UnificationDeferred::EscapingLocal(target.clone())));
                        }
                    }
                    match &prepared.binding {
                        Binding::Local(id) => {
                            if let Some(value) = value {
                                ...
                                telescope.add_param(id.clone(), prepared.user_name, domain, prepared.style);
                            }
                        }
                        Binding::Residual(id) => {
                            ...
                            validation_store.assign(id.clone(), Expr::fvar(fresh.clone()),
                                AssignmentJustification::UserGiven)
                                .map_err(UnificationError::Metavariable)?;
                            telescope.add_param(fresh, prepared.user_name, domain, BinderInfo::Default);
        ...
        for local in telescope.decls().iter().rev() {
            ...
            if let Some(local_value) = &local.value {
                value = Expr::let_e(local.user_name.clone(), local.type_.clone(), local_value.clone(), value, false);
                type_ = Expr::let_e(local.user_name.clone(), local.type_.clone(), local_value.clone(), type_, false);
            } else {
                value = Expr::lam(local.user_name.clone(), local.type_.clone(), value, local.binder_info);
                type_ = Expr::forall_e(local.user_name.clone(), local.type_.clone(), type_, local.binder_info);
            }
        }
...
mod tests {
    ...
    fn name(value: &str) -> Name { Name::from_components([value]) }
    fn nat() -> Expr { Expr::const_(name("Nat"), Vec::new()) }
    fn number(value: u64) -> Expr { Expr::lit(Literal::Nat(NatLit::from_u64(value))) }
    fn budget() -> UnificationBudget { UnificationBudget::new(Budget::for_stack_bytes(1024 * 1024)) }
    fn transaction() -> ElabTxn { ElabTxn::new(bootstrap_nat_environment(budget().kernel).unwrap(), KVMap::new(), 12) }
    fn declare(txn: &mut ElabTxn, text: &str, type_: Expr, kind: MetavarKind) -> MVarId {
        let id = MVarId(name(text));
        txn.mvars.declare(id.clone(), id.0.clone(), type_, txn.lctx.clone(), kind, 0, None);
        id
    }
    ...
    fn partial_alias_is_checked_but_its_residual_is_not_solved() {
        ...
        let report = txn.unify(&Expr::mvar(a.clone()), &Expr::mvar(b.clone()), budget()).unwrap();
        assert_eq!(report.residual_metavariables, vec![b.clone()]);
        assert_eq!(report.kernel_checks, 1);
        assert_eq!(txn.mvars.get_assigned_expr(&a), Some(&Expr::mvar(b.clone())));
    ...
    fn dependent_residual_types_are_abstracted_before_their_values() {
        let mut txn = transaction();
        let t = declare(&mut txn, "type", Expr::sort(Level::one()), MetavarKind::Natural);
        let b = declare(&mut txn, "value", Expr::mvar(t.clone()), MetavarKind::Natural);
        let a = declare(&mut txn, "target", Expr::mvar(t.clone()), MetavarKind::Natural);
        let report = txn.unify(&Expr::mvar(a), &Expr::mvar(b.clone()), budget()).unwrap();
    ...
    fn residual_context_cannot_hide_a_newer_free_variable() {
        ...
        assert!(matches!(txn.unify(&Expr::mvar(a), &Expr::mvar(b), budget()),
            Err(UnificationError::Deferred(UnificationDeferred::EscapingLocal(_)))));
    ...
    fn opaque_residuals_remain_opaque_in_the_real_store() {
        ...
        assert_eq!(report.residual_metavariables, vec![b.clone()]);
        assert!(!txn.mvars.is_assigned(&b));
        assert_eq!(txn.mvars.get_decl(&b).unwrap().kind, MetavarKind::SyntheticOpaque);
    }
    ...
    fn a_failed_conditional_check_defers_instead_of_rejecting_an_unsolved_type() {
        ...
        assert!(matches!(txn.unify(&Expr::mvar(a), &number(9), budget()),
            Err(UnificationError::Deferred(UnificationDeferred::UnresolvedAssignmentType(_)))));
    ...
    fn declaration_type_cycles_are_not_followed_forever() {
        ...
        let b = MVarId(name("b"));
        let c = MVarId(name("c"));
        txn.mvars.declare(b.clone(), b.0.clone(), Expr::mvar(c.clone()), txn.lctx.clone(), MetavarKind::Natural, 0, None);
        txn.mvars.declare(c.clone(), c.0.clone(), Expr::mvar(b.clone()), txn.lctx.clone(), MetavarKind::Natural, 0, None);
        let a = declare(&mut txn, "a", nat(), MetavarKind::Natural);
        let before = txn.mvars.clone();
        assert!(matches!(txn.unify(&Expr::mvar(a), &Expr::mvar(b), budget()),
            Err(UnificationError::Deferred(UnificationDeferred::UnresolvedAssignmentType(_)))));
    ...
    fn dependent_function_patterns_can_retain_a_typed_residual_body() {
        ...
        let f = declare(&mut txn, "function", Expr::forall_e(name("x"), nat(), nat(), BinderInfo::Default), MetavarKind::Natural);
