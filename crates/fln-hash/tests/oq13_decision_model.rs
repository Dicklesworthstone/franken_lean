...
use fln_hash::cartridge::{
    ...
    WarmDefeqEntryV1, WarmDefeqQueryV1, oq13_attachment_policy, oq13_projection, warm_cache_action,
};
...
fn cache_binding_mismatch_refuses_hints_but_not_the_cartridge_transport() {
    ...
    assert!(matches!(
        archive.validate_present_warm_caches(DecodeBudget::unlimited()),
        Outcome::Complete(Err(CartridgeRefusalV1::WarmCacheBindingMismatch { .. }))
    ));
}
