...
//!   directly as its verdict — `tc.rs:3689` answers KR-303 sort definitional
//!   equality with `lt.is_equiv(ls)`, and `tc.rs:3629`/`4248`/`5801` decide
...
//!   `loose_bvar_range() <= k` (`tc.rs:1897`), and the iterative
//!   `abstract_fvar_set` and `replace_fvar` paths return early when
//!   `!has_fvar()` (`tc.rs:5334`/`5529`). An under-reporting flag makes
...
//!
//! ```text
//! cite crates/fln-kernel/src/tc.rs:3689 :: lt.is_equiv(ls)
//! cite crates/fln-kernel/src/tc.rs:3629 :: ExprNode::Sort { level } @@ fn major_to_cnstr_when_structure
//! cite crates/fln-kernel/src/tc.rs:4248 :: ExprNode::Sort { level } @@ fn is_prop
//! cite crates/fln-kernel/src/tc.rs:5801 :: ExprNode::Sort { level } @@ fn finish_infer_proj
//! cite crates/fln-kernel/src/tc.rs:1897 :: e.loose_bvar_range() <= k
//! cite crates/fln-kernel/src/tc.rs:5334 :: !e.has_fvar() || active == 0
//! cite crates/fln-kernel/src/tc.rs:5529 :: if !e.has_fvar() {
