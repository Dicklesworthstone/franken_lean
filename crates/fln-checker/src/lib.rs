...
//!   directly as its verdict — `tc.rs:3228` answers KR-303 sort definitional
//!   equality with `lt.is_equiv(ls)`, and `tc.rs:3168`/`3779`/`5220` decide
...
//!   `loose_bvar_range() <= k` (`tc.rs:1558`), and the iterative
//!   `abstract_fvar_set` and `replace_fvar` paths return early when
//!   `!has_fvar()` (`tc.rs:4753`/`4948`). An under-reporting flag makes
...
//!
//! ```text
//! cite crates/fln-kernel/src/tc.rs:3228 :: lt.is_equiv(ls)
//! cite crates/fln-kernel/src/tc.rs:3168 :: ExprNode::Sort { level } @@ fn major_to_cnstr_when_structure
//! cite crates/fln-kernel/src/tc.rs:3779 :: ExprNode::Sort { level } @@ fn is_prop
//! cite crates/fln-kernel/src/tc.rs:5220 :: ExprNode::Sort { level } @@ fn finish_infer_proj
//! cite crates/fln-kernel/src/tc.rs:1558 :: e.loose_bvar_range() <= k
//! cite crates/fln-kernel/src/tc.rs:4753 :: !e.has_fvar() || active == 0
//! cite crates/fln-kernel/src/tc.rs:4948 :: if !e.has_fvar() {
