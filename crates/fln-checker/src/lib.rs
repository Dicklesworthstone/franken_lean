...
//!   directly as its verdict — `tc.rs:3223` answers KR-303 sort definitional
//!   equality with `lt.is_equiv(ls)`, and `tc.rs:3163`/`3774`/`5205` decide
...
//!   `loose_bvar_range() <= k` (`tc.rs:1553`), and the iterative
//!   `abstract_fvar_set` and `replace_fvar` paths return early when
//!   `!has_fvar()` (`tc.rs:4738`/`4933`). An under-reporting flag makes
...
//!
//! ```text
//! cite crates/fln-kernel/src/tc.rs:3223 :: lt.is_equiv(ls)
//! cite crates/fln-kernel/src/tc.rs:3163 :: ExprNode::Sort { level } @@ fn major_to_cnstr_when_structure
//! cite crates/fln-kernel/src/tc.rs:3774 :: ExprNode::Sort { level } @@ fn is_prop
//! cite crates/fln-kernel/src/tc.rs:5205 :: ExprNode::Sort { level } @@ fn finish_infer_proj
//! cite crates/fln-kernel/src/tc.rs:1553 :: e.loose_bvar_range() <= k
//! cite crates/fln-kernel/src/tc.rs:4738 :: !e.has_fvar() || active == 0
//! cite crates/fln-kernel/src/tc.rs:4933 :: if !e.has_fvar() {
