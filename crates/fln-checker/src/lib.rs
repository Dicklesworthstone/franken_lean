...
//!   directly as its verdict — `tc.rs:3143` answers KR-303 sort definitional
//!   equality with `lt.is_equiv(ls)`, and `tc.rs:3083`/`3694`/`5125` decide
...
//!   `!has_fvar()` (`tc.rs:4658`/`4853`). An under-reporting flag makes
...
//!
//! ```text
//! cite crates/fln-kernel/src/tc.rs:3143 :: lt.is_equiv(ls)
//! cite crates/fln-kernel/src/tc.rs:3083 :: ExprNode::Sort { level } @@ fn major_to_cnstr_when_structure
//! cite crates/fln-kernel/src/tc.rs:3694 :: ExprNode::Sort { level } @@ fn is_prop
//! cite crates/fln-kernel/src/tc.rs:5125 :: ExprNode::Sort { level } @@ fn finish_infer_proj
//! cite crates/fln-kernel/src/tc.rs:1528 :: e.loose_bvar_range() <= k
//! cite crates/fln-kernel/src/tc.rs:4658 :: !e.has_fvar() || active == 0
//! cite crates/fln-kernel/src/tc.rs:4853 :: if !e.has_fvar() {
