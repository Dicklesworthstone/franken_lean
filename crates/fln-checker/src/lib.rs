...
//!   equality with `lt.is_equiv(ls)`, and `tc.rs:3017`/`3628`/`5059` decide
...
//!   `!has_fvar()` (`tc.rs:4592`/`4787`). An under-reporting flag makes
...
//! cite crates/fln-kernel/src/tc.rs:3628 :: ExprNode::Sort { level } @@ fn is_prop
//! cite crates/fln-kernel/src/tc.rs:5059 :: ExprNode::Sort { level } @@ fn finish_infer_proj
//! cite crates/fln-kernel/src/tc.rs:1528 :: e.loose_bvar_range() <= k
//! cite crates/fln-kernel/src/tc.rs:4592 :: !e.has_fvar() || active == 0
//! cite crates/fln-kernel/src/tc.rs:4787 :: if !e.has_fvar() {
