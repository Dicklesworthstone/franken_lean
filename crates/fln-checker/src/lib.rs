...
//!   directly as its verdict — `tc.rs:3182` answers KR-303 sort definitional
//!   equality with `lt.is_equiv(ls)`, and `tc.rs:3122`/`3733`/`5164` decide
...
//!   `!has_fvar()` (`tc.rs:4697`/`4892`). An under-reporting flag makes
...
//!
//! ```text
//! cite crates/fln-kernel/src/tc.rs:3182 :: lt.is_equiv(ls)
//! cite crates/fln-kernel/src/tc.rs:3122 :: ExprNode::Sort { level } @@ fn major_to_cnstr_when_structure
//! cite crates/fln-kernel/src/tc.rs:3733 :: ExprNode::Sort { level } @@ fn is_prop
//! cite crates/fln-kernel/src/tc.rs:5164 :: ExprNode::Sort { level } @@ fn finish_infer_proj
//! cite crates/fln-kernel/src/tc.rs:1528 :: e.loose_bvar_range() <= k
//! cite crates/fln-kernel/src/tc.rs:4697 :: !e.has_fvar() || active == 0
//! cite crates/fln-kernel/src/tc.rs:4892 :: if !e.has_fvar() {
