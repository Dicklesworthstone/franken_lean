...
fn assert_json_private_companion_residual_report(report: &fln_cli::MultiplexerOutput) {
    ...
    assert_eq!(
        cli_source_lines
            ...
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"k2Checked\":false"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths keep K2 unchecked in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"companionPartsLoaded\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit companion-parts loading in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"extensionBlocksObserved\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit extension observation in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
    ...
    assert_eq!(
        cli_source_lines
            ...
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"dependencyOrderDerived\":true"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths derive dependency order in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"')
                    && line.contains(r#"\"privateCliPrivateReportLoopResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit CliPrivateReport loop residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            ...
            .count(),
        2,
        "both check-olean JSON render paths emit CliPrivateReport residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"coreObservablesLoopResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit core-observables loop residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"privateEqDefMatchResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit eq-def/match residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"privateMatchNResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit match-N residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"')
                    && line.contains(r#"\"privateCliPrivateReportMatchResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit CliPrivateReport match residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"')
                    && line.contains(r#"\"privateCliPrivateReportEquationResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit CliPrivateReport equation residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"privateEqDefResiduals\":"#)
            })
            .count(),
        2,
    ...
    assert_eq!(
        cli_source_lines
            ...
            .count(),
        2,
        "both check-olean JSON render paths emit unsafe-rec/sunfold residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"privateSunfoldFResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit sunfold/F residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"')
                    && line.contains(r#"\"privateCliPrivateReportSunfoldFResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit CliPrivateReport sunfold/F residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            ...
            .count(),
        2,
        "both check-olean JSON render paths emit CliPrivateReport F residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"')
                    && line.contains(r#"\"privateCliPrivateReportImplementationAuxResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit CliPrivateReport implementation auxiliaries in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"privateSunfoldResiduals\":"#)
            })
            .count(),
        2,
    ...
    assert_eq!(
        cli_source_lines
            ...
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"privateInitUnsafeRecResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit Init unsafe-rec residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            ...
            .count(),
        2,
        "both check-olean JSON render paths emit Init.Data unsafe-rec residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"')
                    && line.contains(r#"\"privateInitPreludeUnsafeRecResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit Init.Prelude unsafe-rec residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"')
                    && line.contains(r#"\"privateCliPrivateReportUnsafeRecResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit CliPrivateReport unsafe-rec residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"')
                    && line.contains(r#"\"privateCliPrivateReportStandaloneUnsafeRecResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit standalone CliPrivateReport unsafe-rec residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"privateLoopProofResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit loop-proof residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"privateStandaloneProofNResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit standalone proof-N residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"privateProofNResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit proof-N residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"privateInitProofNResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit Init proof-N residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            ...
            .count(),
        2,
        "both check-olean JSON render paths emit Init.Data proof-N residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            ...
            .count(),
        2,
        "both check-olean JSON render paths emit Init.Prelude proof-N residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"')
                    && line.contains(r#"\"privateCliPrivateReportProofResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit CliPrivateReport proof residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"')
                    && line.contains(r#"\"privateCliPrivateReportStandaloneProofNResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit standalone CliPrivateReport proof-N residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"leanNameHashProofResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit Lean.Name hash-proof residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"leanNameBeqMatchResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit Lean.Name beq-match residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"privateLeanNameResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit private Lean.Name residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"privateInitPreludeResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit private Init.Prelude residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"privateInitResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit private Init residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"listToArrayAuxMatchResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit list-to-array auxiliary match residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"')
                    && line.contains(r#"\"privateInitDataListToArrayResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit private Init.Data list-to-array residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"privateInitDataListResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit private Init.Data list residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"privateInitDataResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit private Init.Data residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"')
                    && line.contains(r#"\"coreObservablesSyntaxMatchResiduals\":"#)
            })
            .count(),
        2,
    ...
    assert_eq!(
        cli_source_lines
            ...
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"arrayMapMProofResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit array mapM proof residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"arrayMapMGoResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit array mapM go residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"privateArrayMapMResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit private array mapM residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"privateGoResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit private go residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"') && line.contains(r#"\"stringExtraUnsafeRecResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit string-extra unsafe-rec residuals in concat-item strings",
    );
    assert_eq!(
        cli_source_lines
            .iter()
            .filter(|line| {
                let line = line.trim();
                line.starts_with('"')
                    && line.contains(r#"\"mergeSortCompanionOnlyUnsafeRecResiduals\":"#)
            })
            .count(),
        2,
        "both check-olean JSON render paths emit merge-sort companion-only unsafe-rec residuals in concat-item strings",
    );
    assert_eq!(
        [
            cli_source_lines
    ...
    assert_eq!(
        cli_source_lines
            ...
            .count(),
        1,
        "the FLBC-run renderer keeps execution beside peak stack depth",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
    ...
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim() == r##""bytes: {}\n","##
                    && lines[1].trim() == r##""format version: {}\n","##
            })
            .count(),
        1,
        "the human olean audit renderer keeps byte count beside format version",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
    ...
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim() == r##""lean version: {}\n","##
                    && lines[1].trim() == r##""reference commit: {}\n","##
            })
            .count(),
        1,
        "the human olean audit renderer keeps Lean version beside reference commit",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            ...
            .count(),
        2,
        "both check-olean JSON render paths keep decoded-private counts beside names",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
    ...
    assert_eq!(
        cli_source_lines
            ...
            .count(),
        2,
        "both check-olean JSON render paths keep loop residuals beside companion residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
        ...
        cli_source_lines
            ...
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateCompanionResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{},\"missing\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateCliPrivateReportLoopResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private-companion residuals beside CliPrivateReport loop residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateCliPrivateReportLoopResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateCliPrivateReportResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep CliPrivateReport loop residuals beside report residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateCliPrivateReportResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"coreObservablesLoopResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep report residuals beside core-observables loops",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"coreObservablesLoopResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateEqDefMatchResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep core-observables loops beside eq-def/match residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateEqDefMatchResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateMatchNResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep eq-def/match residuals beside match-N residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateMatchNResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateCliPrivateReportMatchResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep match-N residuals beside CliPrivateReport matches",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateCliPrivateReportMatchResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateCliPrivateReportEquationResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep CliPrivateReport matches beside equations",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateCliPrivateReportEquationResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateEqDefResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep CliPrivateReport equations beside eq-def residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateEqDefResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateEqNResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep eq-def residuals beside eq-N residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
        ...
        cli_source_lines
            ...
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateSunfoldFResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateCliPrivateReportSunfoldFResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep sunfold/F residuals beside CliPrivateReport sunfold/F residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateCliPrivateReportSunfoldFResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateCliPrivateReportFResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep CliPrivateReport sunfold/F residuals beside F residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateCliPrivateReportFResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateCliPrivateReportImplementationAuxResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep CliPrivateReport F residuals beside implementation auxiliaries",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateCliPrivateReportImplementationAuxResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateSunfoldResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep implementation auxiliaries beside sunfold residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateSunfoldResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep sunfold residuals beside unsafe-rec residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateInitUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep unsafe-rec residuals beside Init unsafe-rec residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateInitUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateInitDataUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep Init unsafe-rec residuals beside Init.Data unsafe-rec residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateInitDataUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateInitPreludeUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep Init.Data unsafe-rec residuals beside Init.Prelude unsafe-rec residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateInitPreludeUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateCliPrivateReportUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep Init.Prelude unsafe-rec residuals beside CliPrivateReport unsafe-rec residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateCliPrivateReportUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateCliPrivateReportStandaloneUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep CliPrivateReport unsafe-rec residuals beside standalone unsafe-rec residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateCliPrivateReportStandaloneUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateLoopProofResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep standalone unsafe-rec residuals beside loop-proof residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateLoopProofResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateStandaloneProofNResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep loop-proof residuals beside standalone proof-N residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateStandaloneProofNResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateProofNResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep standalone proof-N residuals beside generic proof-N residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateProofNResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateInitProofNResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep generic proof-N residuals beside Init proof-N residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateInitProofNResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateInitDataProofNResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep Init proof-N residuals beside Init.Data proof-N residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
        ...
        cli_source_lines
            ...
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateCliPrivateReportProofResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateCliPrivateReportStandaloneProofNResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep CliPrivateReport proof residuals beside standalone proof-N residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateCliPrivateReportStandaloneProofNResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"leanNameHashProofResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep standalone proof-N residuals beside Lean.Name hash-proof residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"leanNameHashProofResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"leanNameBeqMatchResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep Lean.Name hash-proof residuals beside beq-match residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"leanNameBeqMatchResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateLeanNameResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep Lean.Name beq-match residuals beside private Lean.Name residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateLeanNameResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateInitPreludeResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private Lean.Name residuals beside private Init.Prelude residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateInitPreludeResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateInitResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private Init.Prelude residuals beside private Init residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateInitResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"listToArrayAuxMatchResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private Init residuals beside list-to-array auxiliary match residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"listToArrayAuxMatchResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateInitDataListToArrayResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep list-to-array auxiliary match residuals beside private Init.Data list-to-array residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateInitDataListToArrayResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateInitDataListResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private Init.Data list-to-array residuals beside private Init.Data list residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateInitDataListResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateInitDataResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private Init.Data list residuals beside private Init.Data residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateInitDataResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"coreObservablesSyntaxMatchResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private Init.Data residuals beside core-observables Syntax-match residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"coreObservablesSyntaxMatchResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateLeanSyntaxResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep core-observables Syntax-match residuals beside private Lean.Syntax residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateLeanSyntaxResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"arrayMapMProofResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private Lean.Syntax residuals beside array mapM proof residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"arrayMapMProofResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"arrayMapMGoResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep array mapM proof residuals beside array mapM go residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"arrayMapMGoResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateArrayMapMResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep array mapM go residuals beside private array mapM residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateArrayMapMResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateGoResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private array mapM residuals beside private go residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateGoResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"stringExtraUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private go residuals beside string-extra unsafe-rec residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"stringExtraUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"mergeSortCompanionOnlyUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep string-extra unsafe-rec residuals beside merge-sort companion-only unsafe-rec residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"mergeSortCompanionOnlyUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"mergeSortInternalMergeUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep merge-sort companion-only unsafe-rec residuals beside merge-sort internal-merge unsafe-rec residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"mergeSortInternalMergeUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateLoopMatchOneResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep merge-sort internal-merge unsafe-rec residuals beside private loop match-one residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateLoopMatchOneResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateLoopMatchNResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private loop match-one residuals beside private loop match-N residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateLoopMatchNResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateLoopUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private loop match-N residuals beside private loop unsafe-rec residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateLoopUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateLoopEqDefResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private loop unsafe-rec residuals beside private loop eq-def residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
        ...
        cli_source_lines
            ...
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateUnaryResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateMergeSortTRUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private unary residuals beside private mergeSortTR unsafe-rec residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateMergeSortTRUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateRunUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private mergeSortTR unsafe-rec residuals beside private run unsafe-rec residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateRunUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateGoUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private run unsafe-rec residuals beside private go unsafe-rec residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateGoUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateMergeTrGoUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private go unsafe-rec residuals beside private mergeTr-go unsafe-rec residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
            .filter(|lines| {
                lines[0].trim()
                    == r##""\"privateMergeTrGoUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
                    && lines[1].trim()
                        == r##""\"privateSplitRevAtGoUnsafeRecResiduals\":{{\"observed\":{},\"names\":{},\"omitted\":{}}},","##
            })
            .count(),
        2,
        "both check-olean JSON render paths keep private mergeTr-go unsafe-rec residuals beside private splitRevAt-go unsafe-rec residuals",
    );
    assert_eq!(
        cli_source_lines
            .windows(2)
    ...
    assert_eq!(
        cli_source_lines
            ...
            .count(),
        2,
        "both check-olean JSON render paths keep core-observables loop unsafe-rec residuals beside the logical-root pair",
    );
...
fn check_olean_reports_private_auxiliaries_from_the_authoritative_companion_part() {
    ...
    assert_eq!(
        ...
        json_usize_field(
            json_object_field(&json.stdout, "decodedPrivateLoopAuxiliaries"),
            "observed",
        ),
        "human: {}\njson: {}",
        human.stdout,
    ...
    let human_artifact_bytes = human_line_suffix(&human.stdout, "artifact bytes: ")
        .parse::<usize>()
        .expect("human artifact-byte count is a usize");
    assert_eq!(
        human_artifact_bytes,
        json_usize_field(&json.stdout, "artifactBytes"),
        "human: {}\njson: {}",
        human.stdout,
        json.stdout,
    );
