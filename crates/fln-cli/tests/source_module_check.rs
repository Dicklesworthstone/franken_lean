...
impl Project {
    fn new() -> Self {
        let path = std::env::temp_dir().join(format!(
            "fln-source-modules-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed),
        ));
    ...
    fn run(&self, args: &[&str]) -> Output {
        Command::new(env!("CARGO_BIN_EXE_fln"))
            .args(["check-source", "--json"]).args(args)
    ...
    fn refused(&self) -> String {
        ...
        assert!(output.stdout.is_empty(), "failed imports exposed a partial receipt");
        let stderr = String::from_utf8(output.stderr).unwrap();
        assert!(stderr.contains("\"schema\":\"fln.source-check/1\""), "{stderr}");
        stderr
    }
}
...
fn checked_libraries_load_nested_imports_records_and_automation_once() {
    ...
    assert_eq!(std::fs::read(p.0.join("Main.lean")).unwrap(), main.as_bytes());
    assert_eq!(std::fs::read(p.0.join("Lib/Common.lean")).unwrap(), common.as_bytes());
...
fn isolated_module_elaboration_preserves_instance_choices() {
    let p = Project::new();
    p.write("Choice.lean", "instance seven : Inhabited Nat := Inhabited.mk 7");
...
fn false_dependency_proofs_cycles_and_missing_imports_refuse_and_recover() {
    ...
    for source in ["import Main", "theorem bad : (0 : Nat) = 1 := by rfl", "#eval 42"] {
...
fn closure_byte_limits_and_multiple_entry_ambiguity_are_fail_closed() {
    ...
    assert!(!output.status.success());
    assert!(output.stdout.is_empty());
    assert!(String::from_utf8(output.stderr).unwrap().contains("one entry file"));
    p.complete();
}
...
fn import_path_traversal_never_becomes_an_outside_file_read() {
    let p = Project::new();
    for module in ["«..».Secret", "«../Secret»", "«/tmp/Secret»", "«C:Secret»", "«A\\B»"] {
        p.write("Main.lean", &format!("import {module}\ndef use := 0"));
        let error = p.refused();
        assert!(error.contains("unsafe filesystem component"), "{module}: {error}");
    }
}
...
fn import_free_batch_behavior_and_root_independent_receipts_are_preserved() {
    ...
    assert!(output.status.success(), "{}", String::from_utf8_lossy(&output.stderr));
    assert!(String::from_utf8(output.stdout).unwrap().contains("\"files\":2"));
}

#[test]
fn shipped_source_library_example_is_checked_by_the_installed_command() {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../examples/native_modules/Main.lean");
    let output = Command::new(env!("CARGO_BIN_EXE_fln"))
        .args(["check-source", "--json"]).arg(path).output().unwrap();
    assert!(output.status.success(), "{}", String::from_utf8_lossy(&output.stderr));
    assert!(String::from_utf8(output.stdout).unwrap().contains("\"files\":3"));
}
