//! Installed local-template execution, serialized replay and failure atomicity.
#![forbid(unsafe_code)]
...
#[test]
fn local_templates_and_known_callbacks_execute_export_and_replay_without_partial_output() {
    let directory = std::env::temp_dir().join(format!(
        "fln-local-callable-templates-{}",
        std::process::id()
    ));
    std::fs::create_dir(&directory).unwrap();
    let source = directory.join("Example.lean");
    let artifact = directory.join("Example.flbc");
    let program = include_str!("../../../examples/native_local_templates.lean");
