...
fn open(uri: &str, text: &str) -> String {
    notify(
        ...
        format!(
            "{{\"textDocument\":{{\"uri\":{},\"version\":1,\"languageId\":\"lean4\",\"text\":{}}}}}",
            q(uri),
            q(text)
        ),
    )
...
fn document(method: &str, uri: &str) -> String {
    notify(
        method,
        format!("{{\"textDocument\":{{\"uri\":{}}}}}", q(uri)),
    )
}
fn wait(uri: &str, version: usize, id: usize) -> String {
    format!(
        "{{\"jsonrpc\":\"2.0\",\"id\":{id},\"method\":\"textDocument/waitForDiagnostics\",\"params\":{{\"uri\":{},\"version\":{version}}}}}",
        q(uri)
    )
}
fn run(binary: &str, args: &[&str], events: &[String]) -> Vec<String> {
    ...
    assert!(
        !messages
            .iter()
            .any(|m| m.contains("malformed diagnostic callback")
                || m.contains("ambiguous diagnostic callback")),
        "{messages:#?}"
    );
    messages
}
fn fln(events: &[String]) -> Vec<String> {
    run(env!("CARGO_BIN_EXE_fln"), &["serve-lsp"], events)
}
fn successes(messages: &[String]) -> Vec<&str> {
    messages
        .iter()
        .filter(|m| m.contains("$/frankenLean/sourceCheck"))
        .map(String::as_str)
        .collect()
}
fn errors<'a>(messages: &'a [String], uri: &str) -> Vec<&'a str> {
    messages
        .iter()
        .filter(|m| {
            m.contains("publishDiagnostics")
                && m.contains(&q(uri))
                && m.contains("\"diagnostics\":[{")
        })
        .map(String::as_str)
        .collect()
}
...
fn an_unsaved_entry_checks_disk_imports_and_textless_save_reuses_the_graph() {
    ...
    let messages = fln(&[
        open(
            &entry,
            "import Lib\ntheorem use (n : Nat) : wrap n = n := by simp",
        ),
...
fn open_imports_override_disk_and_closed_imports_return_to_disk_without_stale_hits() {
    ...
    assert_eq!(
        std::fs::read_to_string(root.join("Lib.lean")).unwrap(),
        "def value := 0"
    );
}
...
fn invalidated_open_imports_block_disk_fallback_and_diagnostic_waits_then_recover() {
    ...
    let repaired = notify(
        ...
        format!(
            "{{\"textDocument\":{{\"uri\":{}}},\"text\":\"def value := 7\"}}",
            q(&lib)
        ),
    );
    let messages = fln(&[
    ...
    assert!(
        messages
            .iter()
            .any(|m| m.contains("\"id\":23") && m.contains("\"error\"")),
        "{messages:#?}"
    );
    assert!(
        messages
            .iter()
            .any(|m| m.contains("\"id\":24") && m.contains("\"result\":{}")),
        "{messages:#?}"
    );
    assert_eq!(successes(&messages).len(), 5, "{messages:#?}");
}
...
fn unrelated_documents_cannot_lend_declarations_and_false_proofs_do_not_poison_recovery() {
    let messages = fln(&[
        ...
        open(
            "untitled:Main.lean",
            "theorem stolen : secret = 7 := by rfl",
        ),
        change(
            "untitled:Main.lean",
            2,
            "theorem falseProof : (0 : Nat) = 1 := by rfl",
        ),
        change(
            "untitled:Main.lean",
            3,
            "theorem repaired : (7 : Nat) = 7 := by rfl",
        ),
    ]);
    assert_eq!(successes(&messages).len(), 2, "{messages:#?}");
    assert_eq!(
        errors(&messages, "untitled:Main.lean").len(),
        2,
        "{messages:#?}"
    );
}
...
fn resource_stops_are_nonanswers_and_do_not_reuse_an_old_success() {
    ...
    let messages = fln(&[
        ...
        change(document_uri, 2, &" ".repeat(1024 * 1024 + 1)),
        wait(document_uri, 2, 31),
        change(document_uri, 3, "theorem valid : (7 : Nat) = 7 := by rfl"),
        wait(document_uri, 3, 32),
    ]);
    assert!(
        messages
            .iter()
            .any(|m| m.contains("diagnosticOutcome") && m.contains("resource")),
        "{messages:#?}"
    );
    assert!(
        messages
            .iter()
            .any(|m| m.contains("\"id\":31") && m.contains("\"error\"")),
        "{messages:#?}"
    );
    assert!(
        messages
            .iter()
            .any(|m| m.contains("\"id\":32") && m.contains("\"result\":{}")),
        "{messages:#?}"
    );
...
fn admission_only_editor_reports_eval_instead_of_running_or_silently_ignoring_it() {
    ...
    assert_eq!(
        errors(&messages, "untitled:NoExecution.lean").len(),
        1,
        "{messages:#?}"
    );
}
