...
impl Live {
    fn start(binary: &str, args: &[&str], decline: bool) -> Self {
        ...
        let reader = std::thread::spawn(move || {
            ...
            while let Ok(Some(bytes)) = transport::read_message(&mut reader) {
                if send.send(String::from_utf8(bytes).unwrap()).is_err() { break; }
            }
        });
        let stderr = std::thread::spawn(move || { let mut bytes = Vec::new(); stderr.read_to_end(&mut bytes).unwrap(); bytes });
        let mut live = Self { child, input: Some(input), output, reader: Some(reader), stderr: Some(stderr), messages: Vec::new() };
        ...
        assert!(!live.messages.iter().any(|m| m.contains("client/registerCapability")));
        ...
        let result = if decline { r#""error":{"code":-32601,"message":"watching disabled"}"# } else { r#""result":null"# };
        live.send(&format!(r#"{{"jsonrpc":"2.0","id":"frankenLean/source-watch/1",{result}}}"#));
        live
    }
    ...
    fn until(&mut self, predicate: impl Fn(&str) -> bool) -> String {
        ...
        loop {
            let message = self.output.recv_timeout(deadline.saturating_duration_since(Instant::now()))
                .unwrap_or_else(|error| panic!("server did not reach the protocol barrier: {error}; {:#?}", self.messages));
    ...
    fn changed(&mut self, uri: &str, kind: usize) {
        self.send(&notify("workspace/didChangeWatchedFiles", format!(r#"{{"changes":[{{"uri":{},"type":{kind}}}]}}"#, q(uri))));
    }
...
fn creation_and_directory_removal_recover_without_reopening_the_importer() {
    ...
    live.send(&open(&main, "import Library.Values\ntheorem use : value = 0 := by rfl"));
    live.barrier(&main, 84);
    std::fs::create_dir(&dir).unwrap();
    std::fs::write(&path, "def value := 0").unwrap();
    live.changed(&uri(&path), 1);
    live.barrier(&main, 85);
    assert_eq!(main_successes(&live.messages, &main).len(), 1, "{:#?}", live.messages);
    assert!(!backup.exists());
    std::fs::rename(&dir, &backup).unwrap();
    live.changed(&uri(&dir), 3);
    live.barrier(&main, 86);
    assert_eq!(errors(&live.messages, &main).len(), 2, "{:#?}", live.messages);
...
fn filesystem_hints_never_replace_open_source_or_unavailable_buffer_authority() {
    ...
    assert!(errors(&live.messages, &main).is_empty(), "{:#?}", live.messages);
    let before = main_successes(&live.messages, &main).len();
    live.send(&notify("textDocument/didChange", format!("{{\"textDocument\":{{\"uri\":{},\"version\":2}},\"contentChanges\":false}}", q(&lib))));
    live.changed(&lib, 2);
    assert!(live.barrier(&main, 90).contains("\"error\""));
    assert_eq!(main_successes(&live.messages, &main).len(), before);
    assert!(live.finish().iter().any(|m| m.contains("disk fallback is forbidden")));
}
...
fn repeated_events_are_coalesced_and_malformed_batches_do_not_partially_recheck() {
    ...
    live.send(&notify("workspace/didChangeWatchedFiles", format!(r#"{{"changes":[{event},{event},{event}]}}"#)));
    ...
    assert_eq!(main_successes(&live.messages, &main).len(), 2);
    std::fs::write(&path, "def value := 1").unwrap();
    live.send(&notify("workspace/didChangeWatchedFiles", format!(r#"{{"changes":[{event},{{"uri":"bad","type":4}}]}}"#)));
    live.barrier(&main, 94);
    assert!(errors(&live.messages, &main).is_empty());
    assert!(live.messages.iter().any(|m| m.contains("event type must be")));
    live.changed(&lib, 2);
...
fn registration_refusal_preserves_editing_and_preconfigured_file_notifications() {
    ...
    let messages = live.finish();
    assert!(messages.iter().any(|m| m.contains("declined Lean source file watching")));
