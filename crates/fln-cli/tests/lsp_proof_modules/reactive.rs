...
fn response_index(messages: &[String], id: usize) -> usize {
    messages
        .iter()
        .position(|m| m.contains(&format!("\"id\":{id},")))
        .unwrap()
}
    ...
    for (binary, args) in [
        ...
        let messages = run(
            ...
            &[
                ...
                change(&lib, 2, "def value := 1"),
                wait(&main, 1, 71),
                change(&lib, 3, "def value := 0"),
                wait(&main, 1, 72),
            ],
        );
        ...
        let repaired = messages
            .iter()
            .rposition(|m| m.contains("sourceCheck") && m.contains(&q(&main)))
            .unwrap();
        assert!(
            response_index(&messages, 71) < repaired && repaired < response_index(&messages, 72)
        );
...
fn transitive_diamond_consumers_refresh_once_without_rechecking_unrelated_files() {
    ...
    for dependent in [&left, &right, &main] {
        assert_eq!(
            successes_for(&messages, dependent).len(),
            2,
            "{messages:#?}"
        );
...
fn invalidated_dependency_fails_same_version_waits_and_full_save_repairs_them_automatically() {
    ...
    let messages = fln(&[
        ...
        notify(
            ...
            format!(
                "{{\"textDocument\":{{\"uri\":{}}},\"text\":\"def value := 0\"}}",
                q(&lib)
            ),
        ),
        wait(&main, 1, 75),
    ...
    assert!(
        messages[response_index(&messages, 74)].contains("\"error\""),
        "{messages:#?}"
    );
    assert!(
        messages[response_index(&messages, 75)].contains("\"result\":{}"),
        "{messages:#?}"
    );
    assert!(
        messages
...
fn removed_imports_and_stale_dependency_messages_do_not_schedule_spurious_checks() {
    ...
    let messages = fln(&[
        ...
        notify(
            "textDocument/didChange",
            format!(
                "{{\"textDocument\":{{\"uri\":{},\"version\":1}},\"contentChanges\":false}}",
                q(&lib)
            ),
        ),
...
fn structural_quoted_names_and_equivalent_file_uri_spellings_keep_dependency_identity() {
    ...
    let messages = fln(&[
        open(
            &main,
            "import «Quoted.Library»\ntheorem use : value = 0 := by rfl",
        ),
        open(&lib, "def value := 1"),
        change(&lib, 2, "def value := 0"),
    ]);
    assert_eq!(errors(&messages, &main).len(), 1, "{messages:#?}");
    assert_eq!(successes_for(&messages, &main).len(), 2, "{messages:#?}");
...
fn rechecking_an_unchanged_importer_version_does_not_satisfy_future_version_waits() {
    ...
    let messages = fln(&[
        ...
        change(&lib, 2, "def value := 0"),
        wait(&main, 1, 79),
        change(&main, 2, source),
    ]);
    assert!(
        response_index(&messages, 79) < response_index(&messages, 78),
        "{messages:#?}"
    );
