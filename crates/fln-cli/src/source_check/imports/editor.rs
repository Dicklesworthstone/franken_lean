...
pub(in crate::source_check) fn document_path(uri: &str) -> Result<PathBuf, Failure> {
    let rest = uri
        .strip_prefix("file://")
        .ok_or_else(|| Failure::input("source imports require a local file URI"))?;
    ...
    if rest.contains(['?', '#']) {
        return Err(Failure::input(
            "file URI queries and fragments are unsupported",
        ));
    }
    ...
    let text =
        String::from_utf8(bytes).map_err(|_| Failure::input("file URI path is not UTF-8"))?;
    if text.contains('\0') {
        return Err(Failure::input("file URI path contains NUL"));
    }
    #[cfg(windows)]
    let text = if text.as_bytes().get(2) == Some(&b':') {
        text[1..].to_owned()
    } else {
        text
    };
    ...
    if !path.is_absolute() {
        return Err(Failure::input("file URI path must be absolute"));
    }
    let parent = path
        .parent()
        .ok_or_else(|| Failure::input("file URI needs a file name"))?;
    ...
    match std::fs::symlink_metadata(&path) {
        Ok(metadata) if metadata.file_type().is_symlink() || !metadata.is_file() => Err(
            Failure::input("editor source is a symlink or not a regular file"),
        ),
        Ok(_) => Ok(path),
...
pub(in crate::source_check) fn load(
    ...
    if text.len() > max_bytes {
        return Err(Failure::resource("editor source exceeds its byte limit"));
    }
    ...
    if entry_path.extension().and_then(|s| s.to_str()) != Some("lean") {
        return Err(Failure::input(
            "source module entry needs a .lean extension",
        ));
    }
    let stem = entry_path
        .file_stem()
    ...
    for document in documents {
        let Ok(path) = document_path(document.uri) else {
            continue;
        };
        ...
        {
            return Err(Failure::input(
                "multiple open document URIs alias one source file",
            ));
        }
    }
    ...
    while cursor < result.sources.len() {
        ...
        for name in header.imports {
            ...
            if by_path.contains_key(&path) {
                return Err(Failure::input("source module paths alias one another"));
            }
            ...
            let (bytes, source_uri) = match overlays.get(&path) {
                Some((source_uri, Some(source))) => {
                    if source.len() > remaining {
                        return Err(Failure::resource(
                            "editor import closure exceeds its byte limit",
                        ));
                    }
                ...
                Some((source_uri, None)) => {
                    return Err(Failure {
                        ...
                        exit: 3,
                        detail: format!(
                            "open imported document {source_uri} has no valid retained source; disk fallback is forbidden"
                        ),
                    });
                }
                None => {
                    let bytes = read_bounded(&path, remaining, "imported editor source").map_err(
                        |error| Failure {
                            class: error.class(),
