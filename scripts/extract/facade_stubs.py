...
The emitter refuses a missing or untyped demanded row. A comment saying a requested
symbol was missing used to create a syntactically valid *partial* facade; that is the
opposite of the census-totality law, because downstream compilation can no longer tell
whether the slice was genuinely complete.
"""
        ...
        for lineno, line in enumerate(fh, 1):
            ...
            if row.get("kind") == "summary":
                if summary is not None:
                    raise SystemExit(
                        "REFUSE: resistance census has multiple summary rows — a "
                        "consumer cannot decide which totality claim defines the facade")
                summary = row
            elif row.get("kind") == "symbol":
                required = ("census_key", "partition")
                missing = [field for field in required if not row.get(field)]
                if missing:
                    raise SystemExit(
                        f"REFUSE: {RESISTANCE}:{lineno} is a stale facade row for "
                        f"{row.get('name')!r}; missing {missing} — regenerate the "
                        "mechanical totality census before emitting stubs")
                rows.append(row)
    if summary is None:
        raise SystemExit("REFUSE: resistance census has no totality summary")
    if summary.get("unclassified") != 0 or summary.get("classified") != summary.get("union_demanded"):
        raise SystemExit(
            "REFUSE: resistance census does not prove a total demanded partition "
            f"(classified={summary.get('classified')!r}, "
            f"union_demanded={summary.get('union_demanded')!r}, "
            f"unclassified={summary.get('unclassified')!r})")
    if not rows:
...
def main():
    ...
    ap.add_argument("--manifest",
                    help="also emit fln-facade-meta-tactic-slice/1 metadata for the "
                         "typed generated facade rows")
    ...
    untyped = sorted(row["name"] for row in rows if row["name"] not in types)
    if missing or untyped:
        missing_rows = sorted(set(missing) | set(untyped))
        raise SystemExit(
            "REFUSE: the pinned environment did not yield a type for every demanded "
            f"facade row: {missing_rows[:8]}{' …' if len(missing_rows) > 8 else ''} — "
            "no partial facade is emitted")
    ...
    if args.manifest:
        # This is a slice manifest, not a second source of truth: every row comes
        # from the resistance census plus the same pinned-environment probe whose
        # pretty-printed signature the generated axiom uses. Keeping the signature
        # and its full Pi telescope together makes the generated surface auditable
        # without hand-maintained façade declarations.
        manifest_schema = "fln-facade-meta-tactic-slice/1"
        manifest_rows = [{
            ...
            "typed": emitted,
            "unclassified": 0,
            "partial_emission": False,
            "support": "L1-shape-compatible",
            "source": "fln-facade-resistance/1",
        }]
        for row in rows:
            name = row["name"]
            signature = typesx[name] if name in explicit_for else types[name]
            manifest_rows.append({
                "schema": manifest_schema,
                "kind": "symbol",
                "name": name,
                "census_key": row.get("census_key"),
                "partition": row.get("partition", "toolchain-api"),
                "bucket": row["bucket"],
                "effect": row.get("effect"),
                "safety": row.get("safety"),
                "extern": row.get("extern"),
                "level_params": levels[name].split(",") if levels[name] else [],
                "signature": signature,
                "telescope": signature,
                "stub": "axiom",
                "support": "L1-shape-compatible",
