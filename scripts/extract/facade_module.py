...
def unreproduced_family_error(decl, provided, line_map, unreproduced,
    ...
    for owner in sorted(unreproduced):
        # THE FAMILY IS THE PIN'S CONSTRUCTOR LIST, not a prefix scan of the rows
        # this run happens to hold. Deriving it from `decl` made this guard blind
        # in the one direction that matters -- a constructor the closure never
        # pulled in simply does not appear, so the scan reports a smaller family
        # and passes -- and made it hostile in another, since any ordinary
        # declaration living under the same namespace would be demanded to be an
        # axiom. The pin says which names are constructors; measured on the
        # published artifact, both owners list 15 and the closure holds exactly
        # those 15, in both directions.
        family = list((pin_inductives.get(owner) or {}).get("ctors", ()))
        if not family:
            bad.append(f"{owner}: the pin lists no constructors for it, yet its "
                       "block was built and accepted")
            continue
        for n in family:
            if n not in decl:
                bad.append(f"{n}: the pin lists it as a constructor of {owner} "
                           "and the closure never pulled it in")
            elif n in provided:
