#!/usr/bin/env bash
# W4 Vellum parser-corpus no-mock evidence lane. The existing Pratt suites are
# run as named targets with anti-vacuity floors and an explicit no-skip check,
# then a local artifact-only driver exercises the public parser over three real
# files. Canonical semantic NDJSON and bounded telemetry stay separate and
# scripts/evidence.py independently validates both before bundle commitment.

set -Eeuo pipefail

PYTHON_BIN="$(command -v python3 || true)"
[ -n "$PYTHON_BIN" ] || {
  echo "[parser_corpus_no_mock_e2e] setup failure: python3 is required" >&2
  exit 2
}
...
if ((${#HOSTILE_PYTHON_CONFIGURATION[@]} > 0)); then
  printf '[parser_corpus_no_mock_e2e] setup failure: sealed_interpreter_hostile_environment names=%s\n' \
    "$(IFS=,; printf '%s' "${HOSTILE_PYTHON_CONFIGURATION[*]}")" >&2
  exit 2
fi
for required_command in cargo setsid sha256sum; do
  command -v "$required_command" >/dev/null 2>&1 || {
    echo "[parser_corpus_no_mock_e2e] setup failure: $required_command is required" >&2
    exit 2
  }
done
...
SCHEMA="fln.e2e/2"
BEAD="franken_lean-urd1"
SCENARIO="parser_corpus_no_mock_e2e"
RUN_ID="parser-corpus-no-mock-$(date -u +%Y%m%dT%H%M%SZ)-$$"
...
DRIVER_BIN="${CARGO_TARGET_DIR:-$ROOT/target/cargo}/e2e-parser-corpus/debug/fln-parser-corpus-e2e-driver"
BUILD_TARGET="${CARGO_TARGET_DIR:-$ROOT/target/cargo}/e2e-parser-corpus"
...
case "$READY_WAIT_MS" in
  ''|*[!0-9]*)
    echo "[parser_corpus_no_mock_e2e] setup failure: FLN_E2E_READY_WAIT_MS must be numeric" >&2
    exit 2
    ;;
esac
...
if ! INPUT_ROOT="$(
  ...
  echo "[parser_corpus_no_mock_e2e] setup failure: cannot hash governed inputs" >&2
  exit 2
fi
...
note() {
  printf '[parser_corpus_no_mock_e2e] %s\n' "$*" | tee -a "$HUMAN" >&2
}
...
publish_early_partial() {
  ...
  if [ "$ART_DIR_CLAIMED" -eq 1 ] && [ -d "$ART_DIR" ]; then
    "${PYTHON[@]}" "$EVIDENCE" publish-partial-bundle --art-dir "$ART_DIR" \
      ...
      --argv-json '["scripts/e2e/parser_corpus_no_mock_e2e.sh"]' --cwd "$ROOT" \
      >/dev/null 2>&1 || true
  fi
  gate_release_note_once
  exit "$FINAL_EXIT"
...
on_exit() {
  ...
  emit_event --string event run_end --string verdict "$FINAL_VERDICT" \
    ...
    --string receipt_root not_applicable_parser_corpus_semantic_schema \
  ...
  if [ "$publish_rc" -ne 0 ]; then
    printf '[parser_corpus_no_mock_e2e] INTERNAL FAULT: incomplete bundle %s\n' \
      "$ART_DIR" >&2
    gate_release_note_once
    exit 2
  fi
  printf '[parser_corpus_no_mock_e2e] %s reason=%s evidence=%s\n' \
...
if ! mkdir "$ART_DIR" 2>/dev/null; then
  trap - EXIT
  echo "[parser_corpus_no_mock_e2e] evidence directory already claimed: $ART_DIR" >&2
  gate_release_note_once
  exit 2
fi
ART_DIR_CLAIMED=1
...
[package]
name = "fln-parser-corpus-e2e-driver"
version = "0.0.0"
edition = "2024"
publish = false
...
const LEFTOVER_DIAGNOSTIC: &str = "unexpected token '='; expected command";
...
impl DriverGrammar {
    fn build(source: &str) -> Result<Arc<DriverGrammar>, String> {
        ...
        if !run.accepted() {
            return Err(format!("lexer refused input: {:?}", run.diagnostics()));
        }
        let tokens = run
            .events
            .iter()
        ...
        let leaves = Leaves::build(&text, &tokens)
            .map_err(|error| format!("token attachment refused input: {error:?}"))?;
    ...
    fn production_at<'a>(
        ...
        let Some(token_index) = self.next_token_index(state.pos()) else {
            return Lookup::TokenError(ParseError::new(
    ...
    fn token_text(&self, token_index: usize) -> String {
...
impl Grammar for DriverGrammar {
    fn kind(&self) -> Name {
        name("term")
    }
    ...
    fn consume_token(&self, state: &mut ParserState) -> Result<String, ParseError> {
        ...
        Ok(self.token_text(token_index))
    }
}
...
fn literal_production(kind: LiteralKind, leaf: Syntax, end: BytePos) -> Production {
    Production::new(name("num"), 0, move |state| {
        if kind != LiteralKind::Nat {
            state.set_error(ParseError::consuming(
                "real-file driver accepts only natural literals",
                state.pos(),
            ));
            return;
        }
        ...
        state.push(Syntax::node(name("num"), vec![leaf.clone()]));
    })
}

fn operator_production(
    grammar: Weak<DriverGrammar>,
    operator: Operator,
    ...
    Production::new(name(&format!("term_{}_", operator.symbol)), 0, move |state| {
        if !state.check_lhs_prec(left_requirement)
            || !state.check_prec(operator.precedence)
        {
            return;
        }
        ...
        let Some(grammar) = grammar.upgrade() else {
            state.set_error(ParseError::consuming(
                "real-file grammar expired during parse",
                state.pos(),
            ));
            return;
        };
        ...
        state.push(Syntax::node(
            name(&format!("term_{}_", operator.symbol)),
...
fn render_tree(node: &Syntax) -> Result<String, String> {
    match node {
        Syntax::Node { kind, args, .. }
            if kind.to_display_string() == "num" && args.len() == 1 =>
        {
            render_tree(&args[0])
        }
        Syntax::Node { args, .. } if args.len() == 3 => {
            let operator = match &args[1] {
                Syntax::Atom { val, .. } => val,
                other => {
                    return Err(format!(
                        "operator node has non-atom middle child: {other:?}"
                    ));
                }
            };
            Ok(format!(
        ...
        Syntax::Atom { val, .. } => Ok(val.clone()),
        other => Err(format!("unsupported driver tree shape: {other:?}")),
    }
}
...
struct Observation {
    accepted: bool,
    boundary_diagnostics: Vec<Diagnostic>,
    parse_stop_byte: usize,
...
fn observe(source: &str) -> Result<Observation, String> {
    let grammar = DriverGrammar::build(source)?;
    let token_count = grammar.tokens.len();
    let mut state = ParserState::new(0);
    pratt_parser(grammar.as_ref(), &mut state);
    let parse_stop_byte = state.pos().0;
    let parser_diagnostics = state
        .error()
        .map(|error| {
            vec![Diagnostic {
                byte_offset: error.at.0,
                message: error.message.clone(),
                origin: "parser",
            }]
        })
        .unwrap_or_default();
    let partial_tree = result_of(&state)
        .ok_or_else(|| "parser produced no retained tree".to_string())
        .and_then(render_tree)?;
    let boundary_diagnostics =
        if parser_diagnostics.is_empty()
            && let Some(token_index) = grammar.next_token_index(state.pos())
        {
            let token = &grammar.tokens[token_index];
            vec![Diagnostic {
                byte_offset: token.extent.start().0,
                message: format!(
                    "unexpected token '{}'; expected command",
                    grammar.token_text(token_index)
                ),
    ...
    Ok(Observation {
        accepted: parser_diagnostics.is_empty() && boundary_diagnostics.is_empty(),
...
fn refuse(message: &str) -> ! {
    eprintln!("parser corpus driver setup failure: {message}");
    process::exit(2);
}
...
fn main() {
    ...
    let observed = observe(source)
        .unwrap_or_else(|error| refuse(&format!("cannot observe parser: {error}")));
    let diagnostic_count =
        observed.parser_diagnostics.len() + observed.boundary_diagnostics.len();
    let expected_tree = if case == "positive" {
        "(1 + (2 * 3))"
    } else {
        "(1 = 2)"
    };
    let expected_stop = if case == "positive" { 9 } else { 5 };
    let expected_accepted = case == "positive";
    let expected_boundary_count = usize::from(!expected_accepted);
    let expected_boundary = observed.boundary_diagnostics.first();
    let trailing_refusal_discarded = !expected_accepted
        && observed.parser_diagnostics.is_empty()
        && observed.boundary_diagnostics.len() == 1
        && observed.partial_tree == expected_tree
        && observed.parse_stop_byte == expected_stop;
    let status = if observed.accepted == expected_accepted
        && observed.partial_tree == expected_tree
        && observed.parse_stop_byte == expected_stop
        && observed.parser_diagnostics.is_empty()
        && observed.boundary_diagnostics.len() == expected_boundary_count
        && (expected_accepted
            || expected_boundary.is_some_and(|diagnostic| {
                diagnostic.byte_offset == 6
                    && diagnostic.message == LEFTOVER_DIAGNOSTIC
                    && diagnostic.origin == "command_boundary"
            }))
    {
        "pass"
    } else {
    ...
    let semantic = format!(
        concat!(
            "{{\"accepted\":{},\"boundary_diagnostics\":[{}],\"case\":{},",
            "\"data_grade\":\"verified\",\"input_artifact\":{},\"input_sha256\":{},",
            "\"parse_stop_byte\":{},\"parser_diagnostics\":[{}],\"partial_tree\":{},",
            "\"run_id\":{},\"schema\":\"fln.e2e.parser-corpus-semantic\",",
            "\"status\":{},\"trailing_refusal_discarded\":{},\"version\":1}}\n"
        ),
        observed.accepted,
        diagnostics_json(&observed.boundary_diagnostics),
        json_string(case),
        json_string(input_artifact),
        json_string(input_sha256),
        observed.parse_stop_byte,
        diagnostics_json(&observed.parser_diagnostics),
        json_string(&observed.partial_tree),
        json_string(run_id),
        json_string(status),
    ...
    let telemetry = format!(
        concat!(
            ...
            "\"max_diagnostics\":{},\"max_input_bytes\":{},\"max_tokens\":{},",
            "\"observed_diagnostics\":{},\"observed_input_bytes\":{},",
            "\"observed_tokens\":{},\"run_id\":{},",
            "\"schema\":\"fln.e2e.parser-corpus-telemetry\",",
    ...
    println!("parser-corpus-semantic case={case} status={status}");
    if status != "pass"
        || diagnostic_count > MAX_DIAGNOSTICS
        || observed.token_count > MAX_TOKENS
    {
        process::exit(1);
    }
}
...
emit_event --new-log --string event run_start \
  --json-value argv '["scripts/e2e/parser_corpus_no_mock_e2e.sh"]' \
  --string cwd "$ROOT" \
  ...
  --string seed parser-real-file-v1 \
  ...
  --json-value budgets "{\"capture_bytes_per_stream\":$CAPTURE_BYTES,\"output_budget_bytes\":$OUTPUT_BUDGET_BYTES,\"step_timeout_ms\":$TIMEOUT_MS,\"kill_grace_ms\":$GRACE_MS,\"max_input_bytes\":4096,\"max_tokens\":32,\"max_diagnostics\":4}" \
  || {
...
stdout_path, stderr_path, step, raw_floor, forbidden_marker, output_path = sys.argv[1:]
text = (
...
if forbidden_marker and forbidden_marker in text:
    raise SystemExit(f"{step}: forbidden typed skip was observed: {forbidden_marker}")
matches = [
    int(passed)
...
report = {
    "floor": floor,
    "forbidden_marker": forbidden_marker or None,
...
run_test_step() {
  ...
  if ! validate_test_floor "$step" "$floor" "$forbidden_marker"; then
    record_failure "$step" libtest_floor_status_or_skip_failed
  fi
  record_step "$step" \
    "cargo-test/pass/floor>=$floor/forbidden=${forbidden_marker:-none}" \
    "$LAST_CLASSIFICATION/wrapper=$LAST_RC/child=$LAST_CHILD_EXIT" \
    "$step.counts.json" "$SUBJECT_BEFORE" "$SUBJECT_AFTER" \
    "$GLOBAL_BEFORE" "$GLOBAL_AFTER"
}

run_test_step parser_category_inventory 5 "SKIP parser_category_inventory:" \
  cargo test --locked -q -p fln-parse --test parser_category_inventory -- --nocapture
run_test_step pratt_precedence_model 6 "" \
  cargo test --locked -q -p fln-parse --test pratt_precedence_model
run_test_step parse_roundtrip_property 3 "" \
  cargo test --locked -q -p fln-parse --test parse_roundtrip_property
run_test_step parser_fuzz 4 "" \
  cargo test --locked -q -p fln-parse --test parser_fuzz

ACTIVE_STEP=build_real_file_driver
snapshot_before "$ACTIVE_STEP"
note "building retained real-file parser driver"
...
run_real_file_case() {
  ...
  snapshot_before "$step"
  note "running real-file parser case=$case"
  ...
  if [ "$LAST_CLASSIFICATION" != pass ] || [ "$LAST_RC" -ne 0 ] \
      ...
      || [ ! -s "$telemetry" ]; then
    record_failure "$step" real_file_parser_execution_failed
  fi
  record_step "$step" \
    "public-pratt-parser/$case/canonical-semantic-and-telemetry" \
    "$LAST_CLASSIFICATION/wrapper=$LAST_RC/child=$LAST_CHILD_EXIT" \
...
snapshot_before "$ACTIVE_STEP"
note "independently validating parser semantics, telemetry, and real inputs"
supervise "$ACTIVE_STEP" "${PYTHON[@]}" "$EVIDENCE" \
  ...
  --artifact-root "$ART_DIR" --output "$ART_DIR/parser.validation.json"
inspect_supervisor "$ACTIVE_STEP"
snapshot_after "$ACTIVE_STEP"
if [ "$LAST_CLASSIFICATION" != pass ] || [ "$LAST_RC" -ne 0 ] \
    || [ "$LAST_CHILD_EXIT" != 0 ] \
    || [ ! -s "$ART_DIR/parser.validation.json" ]; then
  record_failure "$ACTIVE_STEP" independent_semantic_validation_failed
fi
record_step "$ACTIVE_STEP" \
  "parser-validation/positive-failure-recovery/pass" \
  "$LAST_CLASSIFICATION/wrapper=$LAST_RC/child=$LAST_CHILD_EXIT" \
  parser.validation.json "$SUBJECT_BEFORE" "$SUBJECT_AFTER" \
  "$GLOBAL_BEFORE" "$GLOBAL_AFTER"
...
set_final pass all_parser_corpus_no_mock_obligations_satisfied 0
exit 0
