...
Both `[Inhabited A]` and `[i : Inhabited A]` are accepted in signatures. The newest eligible local instance is considered before registered globals. Ordinary parameters such as `(i : Inhabited A)` are not silently promoted to local instances. Source instance declarations require an explicit name and result type. Global priorities are nonnegative decimal `u32` values, defaulting to 1000; higher values precede lower values, and newer registrations precede older ones at equal priority.
